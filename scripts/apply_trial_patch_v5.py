#!/usr/bin/env python3
"""Patch billing_endpoints.py with trial-aware credit provisioning (v5).

Uses content-based markers verified against actual file:
  Start: '        if customer_id and price_id:\n'  (8-space indent)
  End:   '\n                        )\n\n\n        return {'  (logger close + empty lines + return)

Replaces the entire credit-provisioning block with trial-aware logic
preserving exact indentation (4-space standard Python).
"""
import ast
import sys
...
# ── Phase 2: locate block via content markers ───────────────────────────────────
# Block start (8-space indent):
BLOCK_START = "        if customer_id and price_id:\n"
# Block end is the 24-space closing paren of logger.info, followed by
# 2 blank lines then 4-space "return {" from the parent function.
BLOCK_END   = "\n                        )\n\n\n    return {"
...
# End of block = position AFTER the closing ')\n\n\n'
...
# The block should end right before the blank lines + return
...
# I2=12s (# Resolve tier..., level 3)
# I3=16s (if pid ==..., level 4)
# I4=20s (tier_key = tk, level 5)
# I5=24s (tenant_id =, CreditStore kwargs, logger, level 6)
# I6=28s (tenant_id=, amount=, reason=, level 7)
