#!/usr/bin/env python3
"""Patch billing_endpoints.py with trial-aware credit provisioning (v10).

FLAT 2-SPACE INDENTATION (verified from live file — every statement level = 2 spaces):
  S1 = 2 spaces  (if customer_id...)
  S2 = 4 spaces  (body inside if)
  S3 = 6 spaces  (nested if/for)
  S4 = 8 spaces  (deepest body)
...
def main():
    ...
    if idx_start == -1:
        sys.exit(f"ERROR: cannot find block start: {repr(BLOCK_START)}")
    ...
    print(f"  Head: {repr(old_block[:50])}")
    print(f"  Tail: {repr(old_block[-60:])}")

    # ── Sanity checks ──────────────────────────────────────────────────────────
    checks = {
        "has CreditStore": "CreditStore().add_credits" in old_block,
        "not already patched": "is_trialing" not in old_block,
        "has credits_provisioned": "credits_provisioned = credits" in old_block,
    }
    for k, v in checks.items():
        print(f"  [{'PASS' if v else 'FAIL'}] {k}")
    if not all(checks.values()):
        sys.exit(f"Block validation failed: {checks}")

    # ── Phase 3: build new block (flat 2-space indentation) ────────────────────
    S1 = " " * 2   # 2 spaces — ALL statement lines
    S2 = " " * 4   # 4 spaces — continuation/body
    S3 = " " * 6   # 6 spaces — continuation/body
    S4 = " " * 8   # 8 spaces — deepest continuation
    ...
    new_block = (
        ...
        f"{S2}# Persist trial dates when trial subscription starts\n"
        ...
        f'{S4}user.license_key if hasattr(user, "license_key") else tenant_id\n'
        f"{S3})\n"
        f"{S3}if license_info:\n"
        f'{S4}meta = license_info.get("metadata") or {{}}\n'
        f'{S4}if not meta.get("trial_started_at"):\n'
        f"{S4}meta.update(trial_dates)\n"
        f'{S4}meta["trial_status"] = "trial"\n'
        f"{S4}await license_repo.update_license(\n"
        f'{S4}license_info["key_id"], {{"metadata": meta}}\n'
        f"{S4})\n"
        f"{S4}logger.info(\n"
        f'{S4}"Persisted trial dates for license %s: %s",\n'
        f'{S4}license_info.get("key_id"), trial_dates,\n'
        f"{S4})\n"
        "\n"
    ...
    if not all(checks.values()):
        sys.exit(f"FAIL: {[k for k, v in checks.items() if not v]}")
