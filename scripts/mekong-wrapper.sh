...
PROMPT="" INTERACTIVE=false ACTION="run" PIPELINE_STAGES=""
...
--provider) _resolve_provider "$2"; shift 2;;
...
case "$ACTION" in
pipeline)
  cd "$CWD" && exec python3 "$MEKONG_ROOT/src/main.py" --pipeline $PIPELINE_STAGES "$PROMPT"
  ;;
list)
  echo "AI CLI tools:"
