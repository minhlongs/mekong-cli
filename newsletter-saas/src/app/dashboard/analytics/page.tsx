...
const demoAnalytics: AnalyticsData = {
    ...
    issues: [
        { id: "1", subject: "🚀 AI Trends in 2025", sent_at: "2024-12-24", recipients_count: 1247, opens_count: 648, clicks_count: 225, open_rate: 0.52, click_rate: 0.18 },
        { id: "2", subject: "Holiday Special Edition", sent_at: "2024-12-17", recipients_count: 1180, opens_count: 531, clicks_count: 142, open_rate: 0.45, click_rate: 0.12 },
        { id: "3", subject: "Year in Review 2024", sent_at: "2024-12-10", recipients_count: 1098, opens_count: 527, clicks_count: 165, open_rate: 0.48, click_rate: 0.15 },
        { id: "4", subject: "Weekly Update #47", sent_at: "2024-12-03", recipients_count: 998, opens_count: 461, clicks_count: 102, open_rate: 0.41, click_rate: 0.11 },
    ],
};
            ...
            <main className="ml-64 p-8">
                ...
                <div className="grid grid-cols-4 gap-6 mb-8">
                    <div className="glass rounded-xl p-6">
                        ...
                        <div className="text-3xl font-bold">{data.metrics.emails_sent.toLocaleString()}</div>
                        ...
                        {[65, 42, 78, 85, 92, 56, 71, 88, 95, 67, 82, 90].map((h, i) => (
                            <div key={i} className="flex-1 flex flex-col items-center gap-2">
                                ...
                                <span className="text-xs text-gray-500">
                                    {["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][i].slice(0, 1)}
                                </span>
                            </div>
                        ))}
                    </div>
                        ...
                        <tbody>
                            {data.issues.map((issue) => (
                                <tr key={issue.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                                    ...
                                    <td className="px-6 py-4 text-gray-400">{new Date(issue.sent_at).toLocaleDateString()}</td>
                                    <td className="px-6 py-4">{issue.recipients_count.toLocaleString()}</td>
                                    <td className="px-6 py-4">{issue.opens_count.toLocaleString()}</td>
                                    <td className="px-6 py-4">{issue.clicks_count.toLocaleString()}</td>
                                    <td className="px-6 py-4">
                                        <span className={`px-2 py-1 rounded text-sm font-medium ${issue.open_rate >= 0.5 ? "bg-green-500/20 text-green-400" :
                                                issue.open_rate >= 0.3 ? "bg-yellow-500/20 text-yellow-400" :
