...
Signing: HMAC-SHA256 with `MEKONG_JWT_SECRET` env var (32+ bytes).
...
Authorization: Bearer <token>
   ...
   try MEKONG_ADMIN_TOKEN exact match (legacy) ──hit──▶ allow (god scope, "*" org)
                │ miss
                ▼
   try jwt.decode(token, MEKONG_JWT_SECRET) ──invalid sig──▶ 401 invalid
                │ valid
                ▼
...
Founder-only CLI tool, runs locally on M1 (uses `MEKONG_JWT_SECRET` from
host env, NOT shared):
...
1. **Secret in env only** — `MEKONG_JWT_SECRET` never logged, never in
...
6. **Legacy token isolation** — `MEKONG_ADMIN_TOKEN` and
   `MEKONG_JWT_SECRET` are independent env vars; compromise of one
...
# 2. Update launchd plist EnvironmentVariables: MEKONG_JWT_SECRET=<new>
