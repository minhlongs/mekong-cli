...
#   JWT_SECRET=REDACTED (generate below)
#   LLM_API_KEY or one of the provider-specific keys
```

Generate a secure JWT_SECRET=REDACTED:
...
```
JWT_SECRET=REDACTED=<generated-value>
```
...
# JWT_SECRET=REDACTED enforcement tests
...
| Env | Behavior |
|-----|----------|
| `AUTH_ENVIRONMENT=dev` (default) | JWT_SECRET=REDACTED auto-generated per session if not set. Warn logged. |
| `AUTH_ENVIRONMENT=staging` | JWT_SECRET=REDACTED **required**, min 32 bytes. Startup fails if absent. |
| `AUTH_ENVIRONMENT=production` | JWT_SECRET=REDACTED **required**, min 32 bytes. Startup fails if absent. |

**Minimum:** 32 bytes (256 bits). Use `token_urlsafe(32)` or `openssl rand -hex 32`.

**Never** commit JWT_SECRET=REDACTED to git. It belongs in `.env` (gitignored) or your secret manager.

### Required env vars for production

```bash
AUTH_ENVIRONMENT=production
JWT_SECRET=REDACTED=<32+ bytes>          # REQUIRED — fail-fast if missing
...
Set `JWT_SECRET=REDACTED` in your `.env` file:

```bash
echo "JWT_SECRET=REDACTED=$(python3 -c 'import secrets; print(secrets.token_urlsafe(32))')" >> .env
```

Or for CI pipelines, set the `JWT_SECRET=REDACTED` environment variable in your CI secrets.

### `RuntimeError: JWT_SECRET=REDACTED is too short`
...
src/auth/env_validator.py Startup env check (JWT_SECRET=REDACTED guard)
