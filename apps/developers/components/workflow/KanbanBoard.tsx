...
import {
  kanbanApi,
  type KanbanBoard as IKanbanBoard,
  type KanbanColumn,
  type KanbanCard,
  type TaskStatus,
  type TaskPriority,
...
export const KanbanBoard: React.FC<KanbanBoardProps> = ({ boardId = 'default' }) => {
  const {
    ...
    error,
    isLoading,
  } = useSWR<IKanbanBoard>(`/api/kanban/boards/${boardId}`, fetcher)
  ...
  if (isLoading)
    return (
      <div className="p-8 text-center text-[var(--md-sys-color-on-surface)]">Loading board...</div>
    )
  if (error)
    return (
      <div className="p-8 text-center text-[var(--md-sys-color-error)]">Error loading board</div>
    )
  if (!board)
    return (
      <div className="p-8 text-center text-[var(--md-sys-color-on-surface)]">Board not found</div>
    )
            ...
            <div className="flex gap-4">
              <div className="flex flex-col gap-1 flex-1">
                ...
                <select
                  ...
                  onChange={e =>
                    setEditingCard({ ...editingCard, priority: e.target.value as TaskPriority })
                  }
                >
                  <option value="low">Low</option>
                  <option value="medium">Medium</option>
              ...
              <div className="flex flex-col gap-1 flex-1">
                ...
                <select
                  ...
                  onChange={e =>
                    setEditingCard({ ...editingCard, status: e.target.value as TaskStatus })
                  }
                >
...
const KanbanCardComp = ({
  ...
  // Priority colors are applied inline in MD3Card style prop
    ...
    >
      <div onPointerDown={e => controls.start(e)} className="cursor-grab active:cursor-grabbing">
        <MD3Card
          ...
          style={{
            borderLeftColor: `var(--md-sys-color-${card.priority === 'critical' ? 'error' : card.priority === 'high' ? 'tertiary' : card.priority === 'medium' ? 'primary' : 'outline'})`,
          }}
        >
          <div className="mb-2 flex items-start justify-between">
            ...
            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                ...
                className="p-1 rounded-full text-[var(--md-sys-color-outline)] hover:text-[var(--md-sys-color-primary)] hover:bg-[var(--md-sys-color-surface-variant)]"
              >
              ...
              <button
                ...
                className="p-1 rounded-full text-[var(--md-sys-color-outline)] hover:text-[var(--md-sys-color-error)] hover:bg-[var(--md-sys-color-error-container)]"
              >
