      ...
      {displayServices.map(service => (
        <MD3Card key={service.name} className="p-4">
          <div className="flex items-start justify-between">
            <div>
              <div className="text-body-small text-on-surface-variant uppercase tracking-wider mb-1">
                {service.name}
              </div>
              <div className="flex items-center gap-2">
                <StatusIcon status={service.status} />
                <span className="text-title-medium font-bold capitalize text-on-surface">
                  {service.status}
                </span>
              </div>
              {service.message && (
                <div className="mt-2 text-body-small text-error">
                  {service.message}
                </div>
              )}
            </div>
            <Activity size={20} className="text-outline" />
          </div>
