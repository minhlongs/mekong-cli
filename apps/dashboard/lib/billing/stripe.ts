...
export const PRICING_TIERS = {
  FREE: {
    ...
    price: 0,
    currency: 'usd',
    features: ['3 team members', '5 projects', 'Basic analytics', 'Community support'],
    limits: {
      teamMembers: 3,
      projects: 5,
      storage: '1GB',
...
export async function handleWebhookEvent(event: Stripe.Event) {
  switch (event.type) {
    case 'checkout.session.completed': {
      ...
      logger.billing('Checkout completed', { tenantId: session.metadata?.tenantId })
      break
    }
    ...
    case 'customer.subscription.deleted': {
      const subscription = event.data.object as Stripe.Subscription
      logger.billing('Subscription cancelled', { subscriptionId: subscription.id })
      break
    }

    case 'invoice.paid': {
      const invoice = event.data.object as Stripe.Invoice
      logger.billing('Invoice paid', { invoiceId: invoice.id })
      break
    }

    case 'invoice.payment_failed': {
      const invoice = event.data.object as Stripe.Invoice
      logger.billing('Payment failed', { invoiceId: invoice.id })
      break
    }
  }
}
