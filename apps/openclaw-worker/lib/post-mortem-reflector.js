...
function getTopLessons(n = 10) {
	try {
		...
		const lessonsEnd = content.indexOf('\n---', lessonsStart + 10);
		const lessonsSection = lessonsEnd !== -1 ? content.slice(lessonsStart, lessonsEnd) : content.slice(lessonsStart, lessonsStart + 2000); // cap at 2000 chars

		// Extract table rows (lines starting with |, skip header/separator)
		const rows = lessonsSection
			.split('\n')
		...
		if (gotchasStart !== -1) {
			...
			const gotchasSection = gotchasEnd !== -1 ? content.slice(gotchasStart, gotchasEnd) : content.slice(gotchasStart, gotchasStart + 1000);
...
async function enhancedReflect(data) {
	...
	try {
		...
		/* knowledge-synthesizer not available */
	}
