...
function getQueueStats() {
	let dlqCount = 0;
	try {
		dlqCount = fs.readdirSync(DLQ_DIR).filter((f) => f.startsWith('dead_')).length;
	} catch (e) {
...
let incrementingActive = false; // 🔒 Atomic flag for activeCount increment
	...
	try {
		...
		if (result && (result.result === 'failed_to_start' || result.result === 'queued_abort' || result.result === 'pro_limit_hit')) {
			...
			if (qaRetries >= 3) {
				...
				await recordMission({
					project: projectShortNameEarly,
					missionId: missionIdEarly,
					taskFile,
					success: false,
					failureType: result.result,
					duration: durationMs,
					buildResult: { build: false, output: result.result },
					tokensUsed: 0,
				});
				...
				log(
					`${result.result.toUpperCase()}: ${taskFile} — CC CLI not ready or rotated. Waiting ${waitTime / 1000}s then retry (${qaRetries + 1}/3).`,
				);
		...
		// 🔒 If mission was BLOCKED (not executed), wait for active mission to finish
			...
			const projectShortName = projectIdMatch
				? projectIdMatch[1]
			...
			if (isTradingMission(content)) {
				try {
					...
					log(
						`[TRADING-PMH] /trading mission → ${tradingResult.action}: ${tradingResult.reason}${tradingResult.followUpDispatched ? ' [follow-up queued]' : ''}`,
					);
				} catch (e) {
	...
	pollIntervalRef = setInterval(() => {
		try {
			...
			const newTasks = tasks.filter(
				(f) => !queuedSet.has(f) && !processingSet.has(f) && f !== currentTaskFile && !fs.existsSync(path.join(config.PROCESSED_DIR, f)),
			); // 🧬 FIX #17: Skip already-processed
