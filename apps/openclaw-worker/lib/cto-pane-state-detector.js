...
function detectPaneState(output) {
	...
	if (
		/rate-limit-options|You've hit your limit|API Error: 429|"code":"throttling"|quota exceeded|AccountQuotaExceeded|TooManyRequests|exceeded the.*usage quota|Retrying in \d+ seconds/.test(
			output,
		)
	)
		return 'RATE_LIMITED';
	...
	if (
		/Enter to select|↑\/↓ to navigate|Esc to cancel|Yes \(Recommended\)|Type something|Submit answers|Review your answers|Ready to submit/.test(
			output,
		)
	)
		return 'INTERACTIVE';
	...
	if (
		/Running…|Waiting…|Bash\(|Read\(|Write\(|fullstack-developer\(|planner\(|debugger\(|tester\(|code-reviewer\(|researcher\(|hook error|background tasks/i.test(
			output,
		)
	)
		return 'WORKING';
	...
	if (
		...
		!/Searching|Running|Explore|Read \d|Smooshing|Whisking|Bloviating|Churning|Building|Prestidigitating|Flowing|Crafting|Spiraling|Nesting|Puttering|Zigzagging|Perambulating|Hashing|Processing|Unfurling|thinking|thought for|Running…|Waiting…|Bash\(/.test(
			output,
		)
	)
		return 'IDLE';
