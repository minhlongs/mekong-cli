...
async function raasLicenseMiddleware(mission, env = {}) {
	...
	const licenseKey = headers['x-raas-license-key'] || headers['X-RaaS-License-Key'] || mission?.licenseKey || process.env.RAAS_LICENSE_KEY;
...
function getMetrics() {
	return {
		...
		cacheHitRate: _metrics.totalValidations > 0 ? ((_metrics.cacheHits / _metrics.totalValidations) * 100).toFixed(2) + '%' : '0%',
	};
}
