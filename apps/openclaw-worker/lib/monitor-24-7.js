...
function captureState() {
    try {
        const raw = execSync(
            `tmux capture-pane -t ${TMUX_SESSION}:0.0 -p -S -20`,
...
function detectState(output) {
    ...
    const busyPatterns = [
        /Nesting/i, /Puttering/i, /Photosynthesizing/i, /Crunching/i,
        /Saut[eé]ing/i, /Marinating/i, /Churning/i, /Cooking/i,
        /Simmering/i, /Steaming/i, /Grilling/i, /Roasting/i,
        /Osmosing/i, /Computing/i, /Levitating/i, /Indexing/i,
        /\d+\s+local\s+agents?/i,
        /[·✻✢]\s+\w+ing/,
        /\d+[ms]\s+\d+[ms]\s*·\s*[↑↓]/,
        /[↑↓]\s*[\d.]+k?\s*tokens/i,
        /Calling tool/i, /Running command/i, /Running/i,
        /thinking/i, /Pasted text/i, /filesystem\s*[-–—]/i,
        /[⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏]/,
    ];
    ...
    const completionPatterns = [
        /(?:Cooked|Churned|Sautéed|Braised).*for\s+\d+/i,
        /Task completed/i, /Finished in \d+/i,
    ];
...
function poll() {
    ...
    if (missionCount > 0 && Math.round(stateDuration / POLL_INTERVAL_MS) % 10 === 0) {
        log(`📊 SUMMARY: ${missionCount} missions, ${successCount} ✅, ${failCount} ❌, rate=${metric.successRate}%, CPU=${metric.cpuLoad}, RAM=${freeRamMB}MB`);
    }
}
...
function dashboard() {
    ...
    const avgSuccess = recentMetrics.length > 0
        ? Math.round(recentMetrics.reduce((sum, m) => sum + m.successRate, 0) / recentMetrics.length)
        : 'N/A';
