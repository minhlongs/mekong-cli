...
function saveState(state) {
    try { fs.writeFileSync(RL_STATE_FILE, JSON.stringify(state, null, 2)); } catch (e) { /* ignore */ }
}
...
async function sendRewardSignal(missionResult) {
    ...
    try {
        ...
        log(`[OPENCLAW-RL] 🧠 Reward signal sent: ${missionResult.success ? '👍' : '👎'} for ${missionResult.missionId} (total: ${state.totalFeedbacks})`);
    } catch (e) {
