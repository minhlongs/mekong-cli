...
function log(msg) {
	...
	try {
		fs.appendFileSync('/Users/macbookprom1/tom_hum_cto.log', `[${ts}] [tom-hum] [QWEN-AGI] ${msg}\n`);
	} catch (e) {}
}
...
async function searchWithGrounding(query) {
	...
	const resultText = webResults.map((r, i) => `${i + 1}. ${r.title}\n   URL: ${r.url}\n   ${r.snippet || ''}`).join('\n');
	...
	try {
		const result = await callGemini(
			...
			{
				system:
					'You are a research analyst synthesizing web search results. Be thorough, cite sources by URL, and provide actionable insights.',
				maxTokens: 2048,
				timeout: 30000,
			},
		);
		...
		fs.writeFileSync(
			path.join(RESEARCH_DIR, filename),
			JSON.stringify(
				{
					query,
					answer: result.text,
...
async function analyzeCode(code, question) {
	...
	try {
		const result = await callGemini(`Analyze this code and answer: ${question}\n\n\`\`\`\n${code}\n\`\`\``, {
			maxTokens: 4096,
			timeout: 30000,
		});
...
async function deepResearch(topic, projectName) {
	...
	// Final synthesis
	const synthesis = results.map((r, i) => `## ${r.query}\n${r.answer}`).join('\n\n');

	try {
		const report = await callGemini(`Synthesize into an actionable report for "${projectName}":\n\n${synthesis}`, {
...
async function researchErrorWithGemini(errorMessage, projectName) {
	...
	const result = await searchWithGrounding(`How to fix: ${errorMessage} TypeScript React Next.js`);
