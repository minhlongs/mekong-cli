...
function getRecentCommits(projectDir, limit) {
  const raw = safeExec(
    `git log --oneline -${limit || 10} --no-merges`,
    projectDir
  );
  if (!raw) return [];
...
function parseTestStats(projectDir) {
  try {
    ...
    if (jestMatch) {
      return { total: parseInt(jestMatch[1]) + (parseInt(jestMatch[2]) || 0), pass: parseInt(jestMatch[1]), fail: parseInt(jestMatch[2]) || 0 };
    }
...
function buildHandoverReport(projectDir, healthScore, scoutIssues) {
  ...
  const recentCommitList = commits
    .slice(0, 5)
    .map(c => `- \`${c.hash}\` ${c.message}`)
    .join('\n') || '- (không có commit)';

  const issuesSection = scoutIssues && scoutIssues.length > 0
