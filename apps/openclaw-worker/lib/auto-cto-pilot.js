	...
	function scheduleNext() {
		...
		intervalRef = setTimeout(async () => {
			try {
				...
				// 🏛️ HIẾN PHÁP ROIaaS PANE LOCK: P0=Sophia, P1=Well, P2=Algo-trader
				...
				if (tasks.length > 0) {
					...
					// 🏛️ HIẾN PHÁP ROIaaS — Pane routing uses config.PANE_CONFIG
					...
					for (let pIdx = 0; pIdx < TOTAL_PANES; pIdx++) {
						try {
							...
							if (isShellPrompt) {
								...
								log(`[🩺 RESPAWN][P${pIdx}] Shell-only detected — auto-restarting CC CLI (endpoint: ${pConf.endpoint || 'default'})`);
								try {
									const { generateClaudeCommand } = require('./brain-spawn-manager');
									const cmd = generateClaudeCommand('API', pIdx);
									require('child_process').execSync(
										`tmux send-keys -t tom_hum:0.${pIdx} "${cmd}"`,
										{ timeout: 8000 },
									);
							...
							if (llmResult.state === 'busy') {
								...
								if (llmResult.confidence >= 0.8) {
									if (currentCount >= 3) {
										log(`[LLM-VISION][P${pIdx}] LOOP WARNING: freeing resources (NOT stopping pane)`);
										try {
											require('child_process').execSync(`rm -rf ~/.npm/_cacache ~/mekong-cli/node_modules/.cache 2>/dev/null`, { timeout: 2000 });
										} catch (e) {}
										_paneQuestionCounts.set(pIdx, 0);
									} else {
...
async function handleScan(state, project, projectDir) {
	...
	if (errors.length === 0) {
		...
		if (!raasRecentlyDispatched) {
			// 🏛️ HIẾN PHÁP ROIaaS 5-Phase DNA missions — ship code for clients
			// 🔒 GIT REPO LOCK: Each project pushes to its OWN repo only
			// Sophia → monorepo mekong-cli.git (cd root to push)
			// Well → Well.git (cd apps/well to push)
			// Algo → algo-trader.git (cd apps/algo-trader to push)
			const HIEN_PHAP_PREFIX = 'Đọc HIEN-PHAP-ROIAAS.md. Tuân thủ 5 PHASE DNA ROIaaS: GATE→LICENSE_UI→WEBHOOK→METERING→ANALYTICS.';
			const RAAS_MISSIONS = {
				'sophia-ai-factory':
					`/cook:auto:parallel "${HIEN_PHAP_PREFIX} Dự án Sophia (apps/sophia-proposal). cd apps/sophia-proposal && git status. Fix build/lint/test errors. Ship production-ready code. GIT: cd apps/sophia-proposal && git add -A && git commit -m 'feat(sophia): ship RaaS' && git push origin main. CẤM push từ root mekong-cli." --auto`,
				'algo-trader':
					`/cook:auto:parallel "${HIEN_PHAP_PREFIX} Dự án Algo-Trader (apps/algo-trader). Repo riêng: algo-trader.git. cd apps/algo-trader && git status. Fix build/lint/test errors. Ship production-ready code. GIT: cd apps/algo-trader && git add -A && git commit -m 'feat(algo): ship RaaS' && git push origin main. CẤM push từ root mekong-cli." --auto`,
				well: `/cook:auto:parallel "${HIEN_PHAP_PREFIX} Dự án WellNexus (apps/well). Repo riêng: Well.git. cd apps/well && git status. Fix build/lint/test errors. Ship production-ready code. GIT: cd apps/well && git add -A && git commit -m 'feat(well): ship RaaS' && git push origin main. CẤM push từ root mekong-cli." --auto`,
				'mekong-cli':
					`/cook:auto:parallel "${HIEN_PHAP_PREFIX} Dự án mekong-cli core BE+FE. Fix build/lint errors. Ship production-ready code. GIT: git add -A && git commit -m 'feat(core): ship RaaS' && git push origin main." --auto`,
			};
			const mission = RAAS_MISSIONS[project];
			if (mission) {
				const filename = `HIGH_mission_${project}_hien_phap_raas_${Date.now()}.txt`;
				...
				log(`AUTO-CTO [🏛️ HIẾN PHÁP ROIaaS]: ${project} GREEN → Dispatched Hiến Pháp 5-phase DNA mission`);
			} else {
