...
function isLearnableMission(mission) {
    ...
    const complexityHints = [
        /refactor/i, /migration/i, /upgrade/i, /integration/i,
        /authentication/i, /deployment/i, /security/i, /performance/i,
        /i18n/i, /api/i, /database/i, /test.*suite/i,
    ];

    const isComplex = mission.complexity === 'complex' ||
        mission.complexity === 'critical' ||
        complexityHints.some(p => p.test(mission.prompt));
...
function searchKnowledge(query) {
    ...
    try {
        ...
        for (const project of projects) {
            ...
            for (const file of files.slice(-50)) { // Last 50 per project
                try {
