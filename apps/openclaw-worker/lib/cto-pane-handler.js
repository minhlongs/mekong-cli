...
function recordScore(project, scoreResult) {
    try {
        let history = {};
        try { history = JSON.parse(fs.readFileSync(SCORE_HISTORY_FILE, 'utf-8')); } catch { }
...
async function handlePane(pane, output, state, ctx) {
    const {
        session, fallbackModel, lastInjection, paneProgress, log,
        capture, sendKeys, respawn,
        recordTaskCompleted,
    } = ctx;
    ...
    switch (state) {
        ...
        case 'MODEL_UNAVAILABLE': {
            ...
            } catch (e) { log(`P${pane.idx}: Settings update failed: ${e.message}`); }
...
async function _handleIdle(pane, output, ctx) {
    const {
        session, lastInjection, paneProgress, log,
        sendKeys, sendBuf,
        isPaneTaskComplete, recordTaskInjected, recordTaskCompleted,
        isInCooldown, recordInjection,
    } = ctx;
    ...
    if (!global.calculateProjectScore) {
        try { global.calculateProjectScore = require('./project-score-calculator').calculateProjectScore; }
        catch { global.calculateProjectScore = () => ({ total: 0, grade: 'F' }); }
    }
    ...
    if (scoreResult.grade === 'S') {
        ...
        } catch (e) { log(`P${pane.idx}: Handover failed: ${e.message}`); }
    ...
    try {
        ...
        if (ap) {
            ...
            if (r.advanced) { cookCmd = `${r.command} --auto`; log(`P${pane.idx}: PIPELINE [${ap.currentStage}→${r.nextStage}]`); }
            else if (!r.completed) { const c = global.factoryPipeline.getCurrentCommand(ap.id); if (c) cookCmd = `${c.command} --auto`; }
        }
    } catch { }
    ...
    if (!cookCmd) {
        ...
        if (scan.action === 'block') { trackIdle(pane.idx, pane.project); return; }
        if (scan.action === 'debug') cookCmd = scan.debugTask;
    }

    if (!cookCmd) { cookCmd = smartTaskFromReality(pane, log); if (cookCmd) log(`P${pane.idx}: REALITY: ${cookCmd.slice(0, 80)}`); }
    ...
    if (!cookCmd) {
        ...
        else { log(`P${pane.idx}: No tasks for ${pane.project}`); trackIdle(pane.idx, pane.project); return; }
    }
