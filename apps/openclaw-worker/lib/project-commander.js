...
function scoreProjects() {
  ...
  const scores = projects.map((project, idx) => {
    ...
    const errorCount = (projectHealth.buildErrors || 0) + 
                       (projectHealth.lintErrors || 0) + 
                       (projectHealth.testFailures || 0);
    ...
    const lastMission = projectMissions.length > 0 
      ? new Date(projectMissions[projectMissions.length - 1].timestamp || 0).getTime()
      : 0;
    ...
    const score = Math.round(
      (errorCount * 10 + basePriority) * stalenessFactor * failurePenalty
    );
  ...
  state.projectScores = {};
  scores.forEach(s => { state.projectScores[s.project] = s.score; });
  saveState(state);
...
function shareCrossProjectKnowledge() {
  ...
  for (const mission of recentSuccesses) {
    ...
    const alreadyShared = (state.crossProjectFixes || []).some(
      f => f.pattern === fixPattern && f.sourceProject === mission.project
    );
...
`;
        ...
        const taskFile = path.join(
          TASKS_DIR,
          `HIGH_mission_crossfix_${fixPattern}_${otherProject}_${Date.now()}.txt`
        );
