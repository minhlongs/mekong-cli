...
const STAGE_CONFIG = {
	...
	VERIFY: {
		...
		gate: 'all_tests_pass',
		command: (project) =>
			`/cook "${project} — run full test suite, lint, type-check. Fix any failures. Critic review: score must be 8+/10" --auto`,
...
const PIPELINE_STORE = path.join(process.env.HOME || '/tmp', '.config', 'openclaw', 'factory-pipelines.json');
...
function recordFailure(pipelineId) {
	...
	if (pipeline.attempts >= pipeline.maxAttempts) {
		...
		return {
			...
			escalate: true,
			pipeline,
			reason: `Max ${pipeline.maxAttempts} attempts reached at stage ${pipeline.currentStage}`,
		};
	}
...
function isPaneLocked(paneIdx) {
	const data = loadPipelines();
	return Object.values(data.pipelines).some((p) => p.status === 'active' && p.paneIdx === paneIdx);
}
...
const LEARNING_STORE = path.join(process.env.HOME || '/tmp', '.config', 'openclaw', 'factory-learning.json');
