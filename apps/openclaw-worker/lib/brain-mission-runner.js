		...
		while (Date.now() < deadline) {
			...
			switch (state) {
				case 'complete': {
					...
					try {
						require('./learning-engine').recordOutcome(prompt.slice(0, 40), path.basename(projectDir || ''), 'done', elapsedSec);
					} catch (e) {}
...
async function _handleIdleState(
	...
	if (elapsedSec > MIN_MISSION_SECONDS) {
		...
		if (count >= IDLE_CONFIRM_POLLS * 6) {
			...
			tx(`tmux send-keys -t ${TMUX_SESSION} Escape`, TMUX_SESSION);
			try {
				require('./learning-engine').recordOutcome(prompt.slice(0, 40), path.basename(projectDir || ''), 'failed_to_start', elapsedSec);
			} catch (e) {}
