...
const SecretsSchema = z.object({
  ...
  JWT_SECRET=REDACTED: z.string().min(32),
...
const JWT_SECRET=REDACTEDS = [
  process.env.JWT_SECRET=REDACTED_CURRENT!,
  process.env.JWT_SECRET=REDACTED_PREVIOUS!, // Keep for grace period
].filter(Boolean);

function verifyTokenWithRotation(token: string): TokenPayload | null {
  for (const secret of JWT_SECRET=REDACTEDS) {
    try {
