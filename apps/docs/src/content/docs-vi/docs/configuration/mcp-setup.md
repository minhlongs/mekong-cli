...
AgencyOS chuyển việc quản lý các máy chủ MCP (Model Context Protocol) cho **mcp-manager** – một subagent chuyên trách. Cách làm này tách toàn bộ manifest của công cụ ra khỏi agent chính, giúp giữ context gọn nhẹ mà vẫn tận dụng được các tích hợp sâu.
...
AgencyOS sẽ gọi **mcp-manager**, nạp cấu hình MCP, phân tích các công cụ khả dụng, chọn phương án phù hợp nhất, thực thi và trả kết quả về phiên làm việc chính.
...
Ngay cả khi cô lập bằng subagent, việc duyệt quá nhiều công cụ MCP vẫn tốn token. Để giảm chi phí, AgencyOS có thể chuyển phần orchestration nặng nề sang **gemini-cli**, tận dụng một runtime rẻ hơn mà vẫn giữ cuộc trò chuyện chính tập trung.
...
- Kết hợp `/use-mcp` với các lệnh tự động như `/cook`, `/fix`, `/plan` để dùng song song công cụ tùy chỉnh và agent gốc của AgencyOS.
