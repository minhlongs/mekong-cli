...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      const usage = {
        ...
        commands: commands || 0,
        api_calls_remaining:
          limits.monthly_api_calls === -1
            ? -1
            : Math.max(0, limits.monthly_api_calls - (apiCalls || 0)),
        commands_remaining:
          limits.monthly_commands === -1
            ? -1
            : Math.max(0, limits.monthly_commands - (commands || 0)),
      };
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      console.error("Usage record error:", errorMessage);
      return res.status(500).json({ error: errorMessage });
    }
  }
