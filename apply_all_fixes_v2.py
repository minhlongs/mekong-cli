...
def fix_sec005():
    ...
    old = '''JWT_SECRET=REDACTED = "test-secret-for-ci-only-not-for-production"'''
    new = '''JWT_SECRET=REDACTED = os.getenv("JWT_SECRET=REDACTED") or secrets.token_urlsafe(32)'''
...
def fix_high017():
    ...
    if not jwt_secret:
        raise RuntimeError("JWT_SECRET=REDACTED environment variable is required")"""
...
def fix_med023():
    ...
    old = 'JWT_SECRET=REDACTED = os.getenv("JWT_SECRET=REDACTED") or secrets.token_urlsafe(32)'
    new = 'JWT_SECRET=REDACTED = os.getenv("JWT_SECRET=REDACTED_SESSION") or os.getenv("JWT_SECRET=REDACTED") or secrets.token_urlsafe(32)'

    if old in content:
        content = content.replace(old, new, 1)
    else:
        # Try the original pattern
        old2 = 'JWT_SECRET=REDACTED = os.getenv("JWT_SECRET=REDACTED")'
        new2 = 'JWT_SECRET=REDACTED = os.getenv("JWT_SECRET=REDACTED_SESSION") or os.getenv("JWT_SECRET=REDACTED")'
        if old2 in content:
...
def fix_low005():
    ...
    new = """JWT_KEYS: Dict[str, str] = {
        "current": os.getenv("JWT_SECRET=REDACTED", ""),
        "previous": os.getenv("JWT_SECRET=REDACTED_PREVIOUS", ""),
    }"""
    ...
    if old in content:
        ...
        JWT_SECRET=REDACTED = JWT_KEYS["current"]'''
        if old2 in content:
            content = content.replace(old2, new2, 1)
