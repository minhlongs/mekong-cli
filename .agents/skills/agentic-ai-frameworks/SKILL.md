...
def supervisor_node(state: AgentState) -> Command[Literal["researcher", "writer", "reviewer", END]]:
    ...
    llm = ChatAnthropic(model="Codex-opus-4-6")
...
def researcher_node(state: AgentState) -> AgentState:
    llm = ChatAnthropic(model="Codex-sonnet-4-6")
    response = llm.invoke([
...
analyst_agent = Agent(
    model=AnthropicModel("Codex-opus-4-6"),
...
- [ ] Benchmark: token cost per task end-to-end before choosing model (GPT-4o vs Codex Sonnet = 3–5× cost difference)
...
- **Model selection per role** — orchestrators need strong reasoning (Codex Opus, GPT-4o); specialists can use faster/cheaper models (Codex Sonnet, Gemini Flash); 3–5× cost reduction
