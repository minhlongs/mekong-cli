...
export class VibeFlow {
    ...
    toReactFlow(workflowId: string): { nodes: unknown[]; edges: unknown[] } | undefined {
        const workflow = this.workflows.get(workflowId);
        if (!workflow) return undefined;
    ...
    // Execute workflow
    async execute(workflowId: string, input: Record<string, unknown>): Promise<{
        success: boolean;
        output: unknown;
        trace: string[];
    }> {
        ...
        return [
            {
                ...
                action: () => console.log('Adding node...'),
            },
        ];
    }
