...
const { TMUX_SESSION_PRO, TMUX_SESSION_API, tmuxExec, isSessionAlive, capturePane } = require('../_bridge/brain-tmux-controller');
const { startHeartbeat, stopHeartbeat } = require('../reliability/brain-heartbeat');
const { startOutputHashWatchdog, stopOutputHashWatchdog } = require('../reliability/brain-output-hash-stagnation-watchdog');
...
function getCurrentWorkerIdx() {
	return _currentWorkerIdx;
}
function setCurrentWorkerIdx(idx) {
	_currentWorkerIdx = idx;
}
...
function isBrainAlive(paneTarget = `${config.TMUX_SESSION}:0.0`) {
	...
	try {
		const output = execSync(`tmux capture-pane -t ${paneTarget} -p 2>/dev/null`, { encoding: 'utf-8', timeout: 8000 });
		...
		const hasCCCLI = [
			...
			/✻/,
			/Cooking|Brewing|Frosting|Moonwalking|Concocting|Sautéing|thinking|Hatching|Ebbing/i,
		].some((p) => p.test(tail5));
...
function isShellPrompt(output) {
	const { getCleanTail } = require('../_bridge/brain-tmux-controller');
...
function findIdleWorker(sessionName = config.TMUX_SESSION, intent = 'EXECUTION', projectDir = '') {
	...
	if (targetPane === -1) {
		const discovered = Object.entries(paneMap)
			.map(([k, v]) => `P${k}=${v.projectName}`)
			.join(', ');
	...
	try {
		const factory = require('../_bridge/factory-pipeline');
...
function setWorkerLock(idx, missionNum) {
	try {
		fs.writeFileSync(workerLockFile(idx), `mission_${missionNum}_P${idx}_${Date.now()}`);
	} catch {}
}

function clearWorkerLock(idx) {
	try {
		fs.unlinkSync(workerLockFile(idx));
	} catch {}
}
