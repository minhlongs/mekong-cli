...
const { getHandForIntent } = require('../_bridge/hands-registry');
const { log } = require('../_bridge/brain-logger');
const { tmuxExec, isSessionAlive, capturePane } = require('../_bridge/brain-tmux-controller');
const { startHealthServer } = require('../observability/brain-health-server');
...
async function spawnBrain() {
	...
	if (isSessionAlive(TMUX_SESSION)) {
		try {
			...
			const paneCount = parseInt(execSync(`tmux list-panes -t ${sessionOnly} | wc -l`, { encoding: 'utf-8' }).trim());
			if (paneCount >= 3) {
	...
	// 🔒 STRICT 1P1 ROUTING — Each pane MUST cd to its assigned project (from config.PANE_CONFIG)
	const dirP0 = config.PANE_CONFIG[0].dir ? path.join(config.MEKONG_DIR, config.PANE_CONFIG[0].dir) : config.MEKONG_DIR;
	const dirP1 = config.PANE_CONFIG[1].dir ? path.join(config.MEKONG_DIR, config.PANE_CONFIG[1].dir) : config.MEKONG_DIR;
	const dirP2 = config.PANE_CONFIG[2].dir ? path.join(config.MEKONG_DIR, config.PANE_CONFIG[2].dir) : config.MEKONG_DIR;
	const sessionName = TMUX_SESSION.split(':')[0];

	log(`BRAIN: Creating NEW session [${sessionName}] — STRICT 1P1: P0=${config.PANE_CONFIG[0].project} P1=${config.PANE_CONFIG[1].project} P2=${config.PANE_CONFIG[2].project}`);
	...
	tmuxExec(`tmux select-pane -t ${TMUX_SESSION}.0 -T "P0: ${config.PANE_CONFIG[0].project} ONLY"`, TMUX_SESSION);

	tmuxExec(`tmux send-keys -t ${TMUX_SESSION}.1 'cd ${dirP1} && ${cmdApi}' Enter`, TMUX_SESSION);
	tmuxExec(`tmux select-pane -t ${TMUX_SESSION}.1 -T "P1: ${config.PANE_CONFIG[1].project} ONLY"`, TMUX_SESSION);

	tmuxExec(`tmux send-keys -t ${TMUX_SESSION}.2 'cd ${dirP2} && ${cmdApi}' Enter`, TMUX_SESSION);
	tmuxExec(`tmux select-pane -t ${TMUX_SESSION}.2 -T "P2: ${config.PANE_CONFIG[2].project} ONLY"`, TMUX_SESSION);
