...
function isContextExhausted(output) {
	if (!output) return false;
	return /auto-compact:\s*0%/i.test(output) || /Context left.*0%/i.test(output);
}
...
async function checkOutputHash() {
	try {
		...
		for (const paneIdx of [0, 1, 2]) {
			...
			if (hashHistory[paneIdx].length === STAGNATION_THRESHOLD && hashHistory[paneIdx].every((h) => h === hashHistory[paneIdx][0])) {
