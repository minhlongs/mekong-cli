...
function recordOutcome(taskId, project, result, elapsedSec, missionSummary = null) {
	...
	log(
		`Recorded: ${taskId} → ${result} (${isSuccess ? '✅' : '❌'}) (${elapsedSec}s)${outcome.filesChanged ? ` [${outcome.filesChanged} files]` : ''}`,
	);
	return outcome;
}
...
function getSuccessRates() {
	...
	for (const o of outcomes) {
		if (!rates[o.taskId])
			rates[o.taskId] = { runs: 0, successes: 0, rate: 0, avgTime: 0, totalTime: 0, filesChanged: 0, avgFilesPerRun: 0, lastRun: null };
		...
		if (lowerContent.includes(taskId.toLowerCase())) {
			if (stats.runs >= 3 && stats.rate > 80) {
				return {
					...
					reason: `High success (${stats.rate}%) — optimized timeout`,
				};
			}
			if (stats.runs >= 3 && stats.avgTime > 1800) {
				return {
					...
					shouldSkip: false,
					preferredIntent: 'PLAN',
					reason: `Slow task (avg ${stats.avgTime}s) — extended timeout + PLAN intent`,
				};
			}
		}
	}
