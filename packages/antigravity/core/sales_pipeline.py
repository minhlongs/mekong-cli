...
class StartupDeal:
    ...
    def get_total_deal_value(self) -> float:
        ...
        return self.get_annual_retainer() + self.get_equity_value() + self.get_success_fee()
...
class SalesPipeline:
    ...
    def qualify_deal(
        self, deal: StartupDeal, funding_target: float = 0.0, valuation: float = 0.0
    ) -> StartupDeal:
        """Qualify deal with funding and valuation data."""
        deal.funding_target = funding_target
        deal.estimated_valuation = (
            valuation if valuation else funding_target * 4
        )  # 25% dilution assumption
        deal.stage = DealStage.DISCOVERY
        return deal

    def check_win_win_win(
        self, deal: StartupDeal, anh_win: str, agency_win: str, startup_win: str
    ) -> bool:
        """
    ...
    def get_active_deals(self) -> List[StartupDeal]:
        ...
        return [
            d for d in self.deals if d.stage not in [DealStage.CLOSED_WON, DealStage.CLOSED_LOST]
        ]
    ...
    def get_total_arr(self) -> float:
        ...
        return sum(d.get_annual_retainer() for d in self.deals if d.stage == DealStage.CLOSED_WON)

    def get_portfolio_equity_value(self) -> float:
        """Calculate total equity value across portfolio."""
        return sum(d.get_equity_value() for d in self.deals if d.stage == DealStage.CLOSED_WON)

    def get_pending_success_fees(self) -> float:
        """Calculate potential success fees from active deals."""
        return sum(d.get_success_fee() for d in self.get_active_deals())
