...
function changeLine(label: string, current: number, prev: number, unit = ''): void {
  ...
  if (delta > 0) success(line);
  else if (delta === 0) info(line);
  else warn(line);
}

export function registerSalesReportCommand(program: Command): void {
  ...
  report
    ...
    .action(() => {
      ...
      keyValue('Period', 'Today (Asia/Saigon)');
      divider();

      info('Activity');
      info('─'.repeat(40));
      success('  New leads captured:       8');
      info('  Leads contacted:           5');
      info('  Demos booked:              2');
      info('  Proposals sent:            1');
      success('  Deals closed:              1');
      divider();
      ...
      info(`  Running MRR (month):       ${fmtUsd(4_320)}`);
      divider();
  ...
  report
    ...
    .action(() => {
      ...
      keyValue('Last week', '2026-03-09 to 2026-03-15');
      divider();

      info('Metric                     This Week  Change');
      info('─'.repeat(52));
      changeLine('New leads', 31, 24);
      changeLine('Leads contacted', 22, 20);
      changeLine('Demos booked', 7, 5);
      changeLine('Proposals sent', 3, 4);
      changeLine('Deals closed', 2, 1);
      divider();

      info('Revenue Metrics');
      info('─'.repeat(52));
      changeLine('New MRR', 298, 149, '$');
      changeLine('Pipeline value', 8_400, 6_200, '$');
      changeLine('Avg deal size', 149, 149, '$');
      divider();
  ...
  report
    ...
    .action(() => {
      ...
      keyValue('MRR end of month (projected)', fmtUsd(4_571));
      divider();
      ...
      info(`  Net New MRR:        ${fmtUsd(1_744)}`);
      divider();

      info('Customers');
      info('─'.repeat(44));
      info('  Active customers:          29');
      success('  New customers:             12');
      warn('  Churned customers:          3  (churn rate: 10.3%)');
      info('  Expansion upgrades:         2');
      divider();

      info('Revenue by Plan');
      info('─'.repeat(44));
      info(`  Starter ($49/mo):   12 customers   ${fmtUsd(588)}/mo`);
      info(`  Pro ($149/mo):      14 customers   ${fmtUsd(2_086)}/mo`);
      success(`  Enterprise ($499):   3 customers   ${fmtUsd(1_497)}/mo`);
      divider();

      keyValue('ARR (annualized)', fmtUsd(4_571 * 12));
      keyValue('LTV (avg)', fmtUsd(1_340));
      keyValue('CAC (avg)', fmtUsd(87));
  ...
  report
    ...
    .action((opts: { months: string }) => {
      ...
      const baseMrr = 4_571;
      const growthRates = { conservative: 0.12, realistic: 0.20, optimistic: 0.32 };

      heading(`Revenue Forecast — ${months}-Month Horizon`);
      keyValue('Base MRR (March 2026)', fmtUsd(baseMrr));
      keyValue('Model', `Compound monthly growth`);
      divider();

      info('Month        Conservative  Realistic     Optimistic');
      info('─'.repeat(56));
      for (let m = 1; m <= months; m++) {
        const date = new Date(2026, 3 + m - 1, 1);
        const label = date.toLocaleString('en-US', { month: 'short', year: '2-digit' });
        const cons = Math.round(baseMrr * Math.pow(1 + growthRates.conservative, m));
        const real = Math.round(baseMrr * Math.pow(1 + growthRates.realistic, m));
        const opti = Math.round(baseMrr * Math.pow(1 + growthRates.optimistic, m));
        const line = `${label.padEnd(13)} ${fmtUsd(cons).padEnd(14)} ${fmtUsd(real).padEnd(14)} ${fmtUsd(opti)}`;
        if (real >= 10_000) success(line);
        else if (real >= 5_000) info(line);
        else info(line);
      }

      divider();
      const finalReal = Math.round(baseMrr * Math.pow(1 + growthRates.realistic, months));
