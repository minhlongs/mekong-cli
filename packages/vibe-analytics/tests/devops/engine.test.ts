...
import { MetricsEngine } from "../../src/devops/engine/metrics-engine";
import { RepoMetricsData } from "../../src/devops/types/github";

describe("MetricsEngine", () => {
  const engine = new MetricsEngine();

  // Mock Data
  const mockData: RepoMetricsData = {
    pullRequests: [
      {
        number: 1,
        title: "Feature A",
  ...
  it("calculates deployment frequency correctly", () => {
    ...
    expect(report.dora.deploymentFrequency.value).toBeCloseTo(0.29, 2);
    expect(report.dora.deploymentFrequency.rating).toBe("High");
  });
  ...
  it("calculates lead time correctly", () => {
    ...
    const report = engine.calculate(mockData, 7);
    expect(report.dora.leadTimeForChanges.value).toBeCloseTo(0.57, 2);
    expect(report.dora.leadTimeForChanges.rating).toBe("Elite");
  });

  it("calculates change failure rate correctly", () => {
    // 1 hotfix out of 2 releases = 50%
    const report = engine.calculate(mockData, 7);
    expect(report.dora.changeFailureRate.value).toBe(50.00);
    expect(report.dora.changeFailureRate.rating).toBe("Low"); // > 15% is Low/Medium
  });

  it("calculates velocity metrics correctly", () => {
    const report = engine.calculate(mockData, 7);

    // Cycle Time: PR1 (25h), PR2 (2.5h) -> Median 13.75h
    expect(report.velocity.cycleTime.value).toBe(13.75);

    // Pickup Time:
    // PR1: 12:00 - 10:00 = 2h
    // PR2: 10:30 - 10:00 = 0.5h
    // Median = 1.25h
    expect(report.velocity.prPickupTime.value).toBe(1.25);
  });
});
