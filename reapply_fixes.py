...
def fix_med023():
    ...
    with open(BASE / fp) as f:
        content = f.read()
    if "JWT_SECRET=REDACTED_" in content or "service.upper()" in content:
    ...
    new2 = '''        # Try service-specific secret first (e.g., JWT_SECRET=REDACTED_LICENSE_GATE)
        service_secret = os.getenv(f"JWT_SECRET=REDACTED_{service.upper()}")
        if service_secret:
            JWT_SECRET=REDACTED = service_secret
        else:
