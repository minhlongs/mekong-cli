...
SKILL_DIR=""
if [ -d ".agencyos/skills/chrome-devtools/scripts" ]; then
  SKILL_DIR=".agencyos/skills/chrome-devtools/scripts"
elif [ -d "$HOME/.agencyos/skills/chrome-devtools/scripts" ]; then
  SKILL_DIR="$HOME/.agencyos/skills/chrome-devtools/scripts"
fi
cd "$SKILL_DIR"
```
...
node aria-snapshot.js --url https://example.com --output ./.agencyos/chrome-devtools/snapshots/page.yaml
```
...
Skills can exist in **project-scope** or **user-scope**. Priority: project-scope > user-scope.
Store snapshots for analysis in `<project>/.agencyos/chrome-devtools/snapshots/`:

```bash
# Create snapshots directory
mkdir -p .agencyos/chrome-devtools/snapshots

# Capture and store with timestamp
SESSION="$(date +%Y%m%d-%H%M%S)"
node aria-snapshot.js --url https://example.com --output .agencyos/chrome-devtools/snapshots/$SESSION.yaml
```
...
Skills can exist in **project-scope** or **user-scope**. Priority: project-scope > user-scope.
All in `.agencyos/skills/chrome-devtools/scripts/`:
...
Skills can exist in **project-scope** or **user-scope**. Priority: project-scope > user-scope.
For complex automation, write scripts to `<project>/.agencyos/chrome-devtools/tmp/`:

```bash
# Create tmp directory for test scripts
mkdir -p $SKILL_DIR/.agencyos/chrome-devtools/tmp

# Write a test script
cat > $SKILL_DIR/.agencyos/chrome-devtools/tmp/login-test.js << 'EOF'
...
# Run the test
node $SKILL_DIR/.agencyos/chrome-devtools/tmp/login-test.js
```
...
Skills can exist in **project-scope** or **user-scope**. Priority: project-scope > user-scope.
Store screenshots for analysis in `<project>/.agencyos/chrome-devtools/screenshots/`:

```bash
# Basic screenshot
node screenshot.js --url https://example.com --output ./.agencyos/chrome-devtools/screenshots/page.png

# Full page
node screenshot.js --url https://example.com --output ./.agencyos/chrome-devtools/screenshots/page.png --full-page true

# Specific element
node screenshot.js --url https://example.com --selector ".main-content" --output ./.agencyos/chrome-devtools/screenshots/element.png
```
...
node screenshot.js --url https://example.com --output ./.agencyos/chrome-devtools/screenshots/page.png --max-size 3

# Disable compression
node screenshot.js --url https://example.com --output ./.agencyos/chrome-devtools/screenshots/page.png --no-compress
```
...
Store logs for analysis in `<project>/.agencyos/chrome-devtools/logs/<session>/`:

```bash
# Create session directory
SESSION="$(date +%Y%m%d-%H%M%S)"
mkdir -p .agencyos/chrome-devtools/logs/$SESSION

# Capture and store
node console.js --url https://example.com --duration 10000 > .agencyos/chrome-devtools/logs/$SESSION/console.json
node network.js --url https://example.com > .agencyos/chrome-devtools/logs/$SESSION/network.json

# View errors
jq '.messages[] | select(.type=="error")' .agencyos/chrome-devtools/logs/$SESSION/console.json
```
...
node screenshot.js --output ./.agencyos/skills/chrome-devtools/screenshots/debug.png
