...
3. **.agencyos/.env file**
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > .agencyos/.env
   ```

4. **.agencyos/skills/.env file** (shared across all Gemini skills)
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > .agencyos/skills/.env
   ```

5. **Skill directory `.env` file**
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > .agencyos/skills/your-skill/.env
   ```
