...
4. **Create plan folder** (only if creating new):
   ...
   - Update session state: `node .agencyos/scripts/set-active-plan.cjs plans/{date}-plan-name`

## Workflow
1. If creating new plan: Create directory `plans/{date}-plan-name` and run `node .agencyos/scripts/set-active-plan.cjs plans/...`
