...
Scans .agencyos/commands/ directory to build catalog at runtime.
...
CATEGORY_GUIDES = {
    ...
    "config": {
        ...
        "workflow": [
            ("Global", "Set user prefs in `~/.agencyos/.ck.json`"),
            ("Local", "Override per-project in `./.agencyos/.ck.json`"),
...
def discover_commands(commands_dir: Path, prefix: str) -> dict:
    """Scan .agencyos/commands/ and build command catalog."""
    commands = {}
    categories = {}
...
def show_config_guide() -> None:
    ...
    print("- Global: `~/.agencyos/.ck.json` (user preferences)")
    print("- Local: `./.agencyos/.ck.json` (project overrides)")
    print()
    ...
    print("## Quick Start")
    print()
    print("**Global config** (`~/.agencyos/.ck.json`) - your preferences:")
    print("```json")
    print('{')
    ...
    print("```")
    print()
    print("**Local override** (`./.agencyos/.ck.json`) - project-specific:")
    print("```json")
    print('{')
    ...
    print("```bash")
    print("# ~/.agencyos/.ck.json - applies everywhere")
    ...
    print("# Local (.agencyos/.ck.json): issuePrefix = \"JIRA-\"")
...
def main():
    # Find .agencyos/commands directory
    script_path = Path(__file__).resolve()
    claude_dir = script_path.parent.parent  # .agencyos/scripts -> .agencyos
    commands_dir = claude_dir / "commands"

    if not commands_dir.exists():
        print("Error: .agencyos/commands/ directory not found.")
        sys.exit(1)
    ...
    if not data["commands"]:
        print("No commands found in .agencyos/commands/")
        sys.exit(1)
