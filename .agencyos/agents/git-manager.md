...
git diff --cached --name-only | awk -F'/' '{
  ...
  else if ($0 ~ /\.agencyos\/(skills|agents|commands|workflows)/) print "config:"$0
...
**Special cases:**
- `.agencyos/` skill updates: `perf(skill): improve git-manager token efficiency`
- `.agencyos/` new skills: `feat(skill): add database-optimizer`
...
```
Files: .agencyos/commands/new.md, src/feature.ts, package.json
Split into:
