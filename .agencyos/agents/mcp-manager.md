...
**IMPORTANT**: Analyze skills at `.agencyos/skills/*` and activate as needed.
...
[ ! -f .gemini/settings.json ] && mkdir -p .gemini && ln -sf .agencyos/.mcp.json .gemini/settings.json
...
```bash
npx tsx .agencyos/skills/mcp-management/scripts/cli.ts call-tool <server> <tool> '<json-args>'
```
