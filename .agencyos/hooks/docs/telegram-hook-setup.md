...
#### Option C: `.agencyos/.env` (Project-Level Override)
...
#### Option D: `.agencyos/hooks/.env` (Hook-Specific)
...
```bash
chmod +x .agencyos/hooks/telegram_notify.sh
```
...
}' | ./.agencyos/hooks/telegram_notify.sh
```

**Expected output:**
```
Telegram notification sent for Stop event in project mekong-engineer
```
...
📍 Location: `/Users/user/projects/mekong-engineer`
```
...
1. **Verify `.agencyos/config.json` exists and is valid JSON:**
   ```bash
   cat .agencyos/config.json | jq .
   ```

2. **Check hook configuration:**
   ```bash
   cat .agencyos/config.json | jq '.hooks'
   ```

3. **Verify script is executable:**
   ```bash
   ls -l .agencyos/hooks/telegram_notify.sh
...
**Script Location:** `.agencyos/hooks/telegram_notify.sh`

**Configuration:** `.agencyos/config.json`
...
**Claude Code Hooks:** https://docs.agencyos.com/claude-code/hooks
