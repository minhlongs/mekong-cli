...
class UsageTracker:
    ...
    def record_usage(
        self, user_id: str, action: str, agency_id: Optional[str] = None, **metadata
    ) -> bool:
...
class SubscriptionMiddleware:
    ...
    def get_subscription(self, user_id: str) -> Dict[str, Any]:
        ...
        if user_id in self._subscription_cache:
            ...
            if datetime.now() - cached.get("_cached_at", datetime.min) < timedelta(
                minutes=5
            ):
                return cached
        ...
        if self.local_license_path.exists():
            try:
                with open(self.local_license_path, "r", encoding="utf-8") as f:
                    ...
                    subscription.update(
                        {
                            "tier": SubscriptionTier.from_str(
                                local_data.get("tier", "starter")
                            ),
        ...
        if self.db:
            try:
                ...
                logger.debug(
                    f"Could not fetch subscription from Supabase for {user_id}: {e}"
                )
    ...
    def check_limit(self, user_id: str, action: str) -> Dict[str, Any]:
        ...
        limit_map = {
            ...
            "video_gen": (
                "max_daily_video",
                "daily_videos",
            ),  # Note: usage field might need different tracking
        }
    ...
    def check_and_warn_quota(self, user_id: str) -> Dict[str, Any]:
        """
    ...
    def validate_license(
        self, license_key: str, email: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        if self.db:
            try:
                query = (
                    self.db.from_("licenses").select("*").eq("license_key", license_key)
                )
                if email:
    ...
    def format_usage_dashboard(self, user_id: str) -> str:
        ...
        api_line = get_stat_line(
            "API CALLS", usage.get("api_calls", 0), limits.monthly_api_calls
        )
        cmd_line = get_stat_line(
            "COMMANDS", usage.get("commands", 0), limits.monthly_commands
        )
