...
class ClientPortalPresenter:
    ...
    def format_project_details(self, project: Project) -> str:
        ...
        for task in project.tasks:
            status_icon = {"todo": "⏳", "in_progress": "🔄", "review": "👀", "done": "✅"}.get(task.status.value, "❓")
    ...
    def format_invoice_details(self, invoice: Invoice) -> str:
        ...
        status_icon = {"draft": "📝", "sent": "📤", "paid": "✅", "overdue": "⚠️"}.get(invoice.status.value, "❓")
...
"""
    ...
    def format_client_list(self, clients: List[Client]) -> str:
        ...
        for client in clients:
            ...
            lines.append(f"   Monthly: ${client.monthly_retainer:,.2f} | Total: ${client.total_spent:,.2f}")
            lines.append("")
    ...
    def validate_invoice_data(self, amount: float, items: List[Dict[str, Any]]) -> List[str]:
        ...
        if abs(total_items_amount - amount) > 0.01:
            errors.append(f"Item amounts (${total_items_amount:.2f}) must equal invoice amount (${amount:.2f})")
