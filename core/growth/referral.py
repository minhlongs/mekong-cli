...
class ReferralSystem:
    ...
    def register_referral(
        self, r_name: str, r_email: str, ref_name: str, ref_email: str, ref_company: str
    ) -> Referral:
        """Register a new referral in the system."""
        ref = Referral(
            id=f"REF-{uuid.uuid4().hex[:6].upper()}",
            referrer_name=r_name,
            referrer_email=r_email,
            referred_name=ref_name,
            referred_email=ref_email,
            referred_company=ref_company,
    ...
    def convert_referral(self, ref_id: str, deal_value: float) -> bool:
        ...
        if ref_id not in self.referrals:
            return False
    ...
    def format_dashboard(self) -> str:
        ...
        for r in list(self.referrals.values())[:5]:
            ...
            lines.append(
                f"║  {status_icon} {r.referrer_name[:15]:<15} → {r.referred_name[:15]:<15} │ ${r.commission_amount:>8,.0f} ║"
            )
