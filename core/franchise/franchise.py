...
class FranchiseSystem:
    ...
    def _create_demo_data(self):
        ...
        self.onboard_franchisee(
            "Minh Nguyen", "minh@agency.vn", "Minh Digital", FranchiseTier.FRANCHISE
        )

    def onboard_franchisee(
        self, name: str, email: str, company: str, tier: FranchiseTier = FranchiseTier.FRANCHISE
    ) -> Franchisee:
    ...
    def claim_territory(self, franchisee_id: str, territory_id: str) -> bool:
        ...
        if franchisee_id not in self.franchisees:
            return False
    ...
    def format_dashboard(self) -> str:
        ...
        claimed_t = sum(
            1 for t in self.territories.values() if t.status != TerritoryStatus.AVAILABLE
        )
        ...
        for f in list(self.franchisees.values())[:5]:
            ...
            lines.append(
                f"║  {tier_icon} {f.company[:20]:<20} │ {len(f.territories)} terr │ ${f.monthly_fee:>8,.0f}/mo ║"
            )
