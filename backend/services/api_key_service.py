...
class ApiKeyService:
    ...
    def rotate_api_key(self, key_id: str, user_id: str, grace_period_hours: int = 24) -> ApiKeyResponse:
        """
        Rotate a key:
        ...
        try:
            # Get old key
            old_key_res = self.db.table("api_keys").select("*").eq("id", key_id).eq("user_id", user_id).execute()
