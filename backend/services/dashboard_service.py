...
class DashboardService:
    ...
    async def create_dashboard(self, user_id: UUID, config: DashboardConfigCreate) -> DashboardConfig:
    ...
    async def update_dashboard(self, dashboard_id: UUID, user_id: UUID, config: DashboardConfigUpdate) -> DashboardConfig:
    ...
    async def get_metric_data(self, metric: str, date_range: str, segment: Optional[str] = None) -> MetricResponse:
        """
        ...
        for i in range(days):
            ...
            # Add some randomness
            import random
            daily_val = base_value * (0.8 + random.random() * 0.4) * (1 + i/days * 0.2) # Upward trend
            data_points.append(
                MetricDataPoint(
                    timestamp=date,
                    value=daily_val,
