...
class OAuthClient(Base):
    ...
    redirect_uris: Mapped[List[str]] = mapped_column(JSON, nullable=False) # List of valid redirect URIs
    scopes: Mapped[List[str]] = mapped_column(JSON, nullable=False) # List of allowed scopes
    grant_types: Mapped[List[str]] = mapped_column(JSON, nullable=False) # e.g. ["authorization_code", "refresh_token"]
    is_confidential: Mapped[bool] = mapped_column(Boolean, default=True) # Public (native app) vs Confidential (server)
...
class OAuthToken(Base):
    ...
    access_token_jti: Mapped[str] = mapped_column(String(255), unique=True, index=True, nullable=False)

    # Refresh token (hashed)
    refresh_token_hash: Mapped[Optional[str]] = mapped_column(String(255), unique=True, index=True, nullable=True)

    client_id: Mapped[str] = mapped_column(ForeignKey("oauth_clients.client_id"))
    user_id: Mapped[str] = mapped_column(String(255), nullable=False, index=True) # Supabase User ID
