...
async def get_franchise_manager() -> Dict[str, Any]:
    ...
    active_territory_values = set(
        f.territory.value for f in demo_franchise.franchisees if f.status.value == "active"
    )
...
async def get_data_moat() -> Dict[str, Any]:
    ...
    return {**strength, "best_practices": practices, "timestamp": datetime.now().isoformat()}
