...
class InvoiceManager:
    ...
    async def handle_payment_failed(self, invoice_obj: Dict[str, Any]):
        ...
        logger.warning(
            f"Payment failed for invoice {invoice_id} (Customer: {customer_id}, Amount: {amount_due})"
        )
        ...
        if self.db and user_id:
            ...
            logger.warning(
                f"Could not send notification: No user_id found in invoice metadata for {invoice_id}"
            )
