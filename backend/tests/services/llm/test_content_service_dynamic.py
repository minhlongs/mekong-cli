...
async def test_generate_blog_post_with_db_prompt(db_session, mock_llm_service, mock_prompt_service):
    ...
    result = await service.generate_blog_post(
        db=db_session, topic="AI", tone="witty", length="short"
    )

    assert result == "Generated Content"

    # Verify prompt service was called
    mock_prompt_service.get_prompt_by_slug.assert_called_once_with(
        db_session, "blog-post-generator"
    )
    ...
    assert (
        call_args.kwargs["system_instruction"]
        == "Custom instruction for witty tone and short length."
    )
...
async def test_generate_blog_post_fallback(db_session, mock_llm_service, mock_prompt_service):
    ...
    service = ContentService()
    result = await service.generate_blog_post(
        db=db_session, topic="AI", tone="professional", length="medium"
    )
    ...
    expected_instruction = PromptTemplates.BLOG_POST_SYSTEM.value.format(
...
async def test_generate_social_caption_dynamic(db_session, mock_llm_service, mock_prompt_service):
    mock_prompt = Prompt(
        slug="social-media-caption", content="Custom social prompt for {platform}", is_active=True
    )
    mock_prompt_service.get_prompt_by_slug.return_value = mock_prompt

    service = ContentService()
    await service.generate_social_media_caption(
        db=db_session, content_description="New Feature", platform="twitter"
    )

    mock_llm_service.generate_text.assert_called_once()
    assert (
        mock_llm_service.generate_text.call_args.kwargs["system_instruction"]
        == "Custom social prompt for twitter"
    )


@pytest.mark.asyncio
async def test_optimize_seo_dynamic(db_session, mock_llm_service, mock_prompt_service):
    mock_prompt = Prompt(slug="seo-optimizer", content="Custom SEO instructions", is_active=True)
    mock_prompt_service.get_prompt_by_slug.return_value = mock_prompt

    service = ContentService()
    await service.optimize_seo(db=db_session, content="Original content")

    mock_llm_service.generate_text.assert_called_once()
    assert (
        mock_llm_service.generate_text.call_args.kwargs["system_instruction"]
        == "Custom SEO instructions"
    )
