...
def orchestrator(mock_email_service, mock_push_service, mock_template_service, mock_redis_service):
    ...
    with patch("backend.services.notification_orchestrator.get_email_service", return_value=mock_email_service), \
         patch("backend.services.notification_orchestrator.PushNotificationService", return_value=mock_push_service), \
         patch("backend.services.notification_orchestrator.get_template_service", return_value=mock_template_service), \
         patch("backend.services.notification_orchestrator.get_notification_rate_limiter") as mock_get_limiter:
...
@pytest.mark.asyncio
async def test_send_notification_happy_path(orchestrator, mock_db_session, mock_email_service, mock_push_service):
