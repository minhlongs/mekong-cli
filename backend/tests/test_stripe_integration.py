...
def stripe_client(mock_stripe):
    with patch.dict(
        "os.environ", {"STRIPE_SECRET_KEY": "sk_test_123", "STRIPE_PUBLISHABLE_KEY": "pk_test_123"}
    ):
        return StripeClient()
...
def test_create_checkout_session(stripe_client, mock_stripe):
    mock_stripe.checkout.Session.create.return_value = MagicMock(
        id="cs_123", url="https://stripe.com/checkout"
    )
...
def test_subscription_manager_create_checkout():
    ...
    with patch(
        "backend.core.payments.subscription_manager.StripeClient", return_value=mock_stripe_client
    ):
...
def test_invoice_manager_list_invoices():
    ...
    with patch(
        "backend.core.payments.invoice_manager.StripeClient", return_value=mock_stripe_client
    ):
...
async def test_webhook_handler_checkout_completed():
    ...
    with (
        patch(
            "backend.core.payments.webhook_handler.StripeClient", return_value=mock_stripe_client
        ),
        patch(
            "backend.core.payments.webhook_handler.ProvisioningService",
            return_value=mock_provisioning,
        ),
        patch(
            "backend.core.payments.webhook_handler.LicenseGenerator", return_value=mock_licensing
        ),
    ):
