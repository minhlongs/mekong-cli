...
def mock_redis_infrastructure():
    ...
    with patch(
        "backend.services.cache.CacheFactory.get_redis", new_callable=AsyncMock
    ) as mock_get_redis:
        ...
        with (
            patch(
                "backend.services.landing_page_service.redis.from_url", return_value=mock_sync_redis
            ),
            patch(
                "backend.api.services.admin_service.redis.from_url", return_value=mock_sync_redis
            ),
        ):
            yield {"async_redis": mock_async_redis, "sync_redis": mock_sync_redis}
...
def test_dashboard_metrics_caching(client, mock_redis_infrastructure):
    ...
    with patch(
        "backend.services.dashboard_service.DashboardService.get_metric_data"
    ) as mock_get_data:
...
def test_landing_page_invalidation(mock_redis_infrastructure):
    ...
    # Mock DB returns
    mock_db.query.return_value.filter.return_value.first.return_value = LandingPage(
    ...
    with (
        patch.object(service.invalidator, "invalidate_pattern") as mock_inv_pattern,
        patch.object(service.invalidator, "invalidate_key") as mock_inv_key,
    ):
        # Create -> Should invalidate list:*
        service.create_landing_page(
            LandingPageCreate(
                title="New", slug="new", content_json={}, seo_metadata={}, template_id="1"
            )
        )
