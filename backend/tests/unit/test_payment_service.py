...
def payment_service():
    ...
    with (
        patch("backend.services.payment_service.PayPalSDK"),
...
class TestCreateCheckoutSession:
    ...
    def test_create_paypal_order_success(self, payment_service):
        ...
        result = payment_service.create_checkout_session(
            provider="paypal", amount=100.0, currency="USD", mode="payment"
        )
    ...
    def test_create_paypal_subscription_missing_plan_id(self, payment_service):
        ...
        with pytest.raises(ValueError, match="PayPal Plan ID .* is required"):
            payment_service.create_checkout_session(
                provider="paypal", amount=0, mode="subscription"
            )
    ...
    def test_create_stripe_checkout_not_configured(self, payment_service):
        ...
        with pytest.raises(ValueError, match="Stripe is not configured"):
            payment_service.create_checkout_session(provider="stripe", amount=100.0)

    def test_unsupported_provider(self, payment_service):
        """Test error with unsupported payment provider."""
        # Act & Assert
        with pytest.raises(ValueError, match="Unsupported provider: unknown"):
            payment_service.create_checkout_session(provider="unknown", amount=100.0)
...
class TestCaptureOrder:
    ...
    def test_capture_paypal_order_success(self, payment_service):
        ...
        # Act
        result = payment_service.capture_order(provider="paypal", order_id="ORDER123")
    ...
    def test_capture_order_failed(self, payment_service):
        ...
        # Arrange
        payment_service.paypal.orders.capture = Mock(side_effect=Exception("Payment declined"))

        # Act & Assert
        with pytest.raises(Exception, match="Payment declined"):
            payment_service.capture_order(provider="paypal", order_id="ORDER123")
...
class TestWebhookVerification:
    ...
    def test_verify_paypal_webhook_success(self, payment_service):
        ...
        event_data = {"event_type": "PAYMENT.CAPTURE.COMPLETED", "resource": {"id": "ORDER123"}}
        ...
        result = payment_service.verify_webhook(
            provider="paypal", headers=headers, body=event_data, webhook_secret="webhook_id_123"
        )
...
class TestWebhookEventHandling:
    ...
    def test_handle_subscription_activated_event(self, payment_service):
        ...
        event = {
            ...
            "resource": {"id": "SUB123", "plan_id": "PLAN-123", "custom_id": "tenant_123"},
        }
    ...
    def test_handle_unknown_event_type(self, payment_service):
        ...
        # Arrange
        event = {"event_type": "UNKNOWN.EVENT.TYPE", "resource": {}}
...
class TestEdgeCases:
    ...
    def test_create_order_with_negative_amount(self, payment_service):
        ...
        with pytest.raises(ValueError):
            payment_service.create_checkout_session(
                provider="paypal", amount=-100.0, mode="payment"
            )

    def test_create_order_with_unsupported_currency(self, payment_service):
        """Test creating order with unsupported currency."""
        payment_service.paypal.orders.create = Mock(side_effect=Exception("Currency not supported"))

        with pytest.raises(Exception, match="Currency not supported"):
            payment_service.create_checkout_session(
                provider="paypal", amount=100.0, currency="INVALID", mode="payment"
            )
...
def test_supported_currencies(payment_service, currency, expected):
    ...
    try:
        payment_service.create_checkout_session(
            provider="paypal", amount=100.0, currency=currency, mode="payment"
        )
        result = True
    except Exception:
        result = False
...
class TestPaymentFlowIntegration:
    ...
    def test_complete_paypal_checkout_flow(self, payment_service):
        ...
        payment_service.paypal.orders.capture = Mock(
            return_value={"id": "ORDER123", "status": "COMPLETED"}
        )

        capture_result = payment_service.capture_order(provider="paypal", order_id="ORDER123")
    ...
    def test_subscription_lifecycle(self, payment_service):
        ...
        payment_service.paypal.subscriptions.create = Mock(
            return_value={"id": "SUB123", "status": "APPROVAL_PENDING"}
        )

        sub_result = payment_service.create_checkout_session(
            provider="paypal", amount=0, price_id="PLAN-123", mode="subscription"
        )
        ...
        activation_event = {
            "event_type": "BILLING.SUBSCRIPTION.ACTIVATED",
            "resource": {"id": "SUB123", "plan_id": "PLAN-123"},
        }
