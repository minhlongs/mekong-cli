...
def test_list_webhook_configs(client, mock_admin_service):
    ...
    mock_admin_service.list_webhook_configs = MagicMock(
        return_value=[
            {
...
def test_create_webhook_config_admin(client, mock_admin_service):
    ...
    mock_admin_service.create_webhook_config = MagicMock(
        return_value={"status": "success", "id": "new-uuid", "message": "Webhook created"}
    )

    app.dependency_overrides[require_admin] = lambda: True
    try:
        response = client.post(
            "/admin/webhooks/configs", json={"url": "https://new.example.com", "event_types": ["*"]}
        )
    finally:
