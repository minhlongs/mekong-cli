...
async def get_webhook_config(
    ...
    result = db.table("webhook_configs").select("*").eq("id", config_id).eq("api_key_id", key_id).execute()
...
async def update_webhook_config(
    ...
    # Verify ownership
    existing = db.table("webhook_configs").select("id").eq("id", config_id).eq("api_key_id", key_id).execute()
    if not existing.data:
        raise HTTPException(status_code=404, detail="Webhook config not found")

    data = update_data.dict(exclude_unset=True)
    if not data:
        return WebhookConfigResponse(**db.table("webhook_configs").select("*").eq("id", config_id).single().execute().data)
...
async def list_deliveries(
    ...
    result = db.table("webhook_deliveries")\
        .select("*")\
