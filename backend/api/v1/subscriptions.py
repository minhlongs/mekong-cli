...
async def list_subscriptions(
    ...
    for record in result.data:
        subs.append(SubscriptionPublic(
            id=record.get("id", "sub_" + record.get("tenant_id", "unknown")), # Handle if ID missing
...
async def get_subscription(
    ...
    result = db.table("subscriptions")\
        ...
        .or_(f"id.eq.{subscription_id},stripe_subscription_id.eq.{subscription_id},paypal_subscription_id.eq.{subscription_id}")\
        .execute()
...
async def cancel_subscription(
    ...
    if not provider_sub_id:
        raise HTTPException(status_code=400, detail="Subscription does not have a valid provider ID")
