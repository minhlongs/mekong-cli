...
# Tenant Metrics
active_tenants = PromGauge("active_tenants_total", "Number of active tenants")
...
# System Metrics
cpu_usage = PromGauge("cpu_usage_percent", "CPU usage percentage")

memory_usage = PromGauge("memory_usage_bytes", "Memory usage in bytes")
...
class MetricsCollector:
    ...
    def record_request(self, request: Request, response: Response, duration: float):
        ...
        http_request_duration.labels(method=method, endpoint=endpoint, tenant_id=tenant_id).observe(
            duration
        )
