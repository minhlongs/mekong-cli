...
@router.get(
    "/feature-flags", response_model=List[FeatureFlag], dependencies=[Depends(require_viewer)]
)
async def list_feature_flags(service: AdminService = Depends(get_admin_service)):
    """List all feature flags."""
    return service.list_feature_flags()


@router.post(
    "/feature-flags", response_model=FeatureFlag, dependencies=[Depends(require_developer)]
)
async def create_feature_flag(
    flag: FeatureFlagCreate, service: AdminService = Depends(get_admin_service)
):
...
@router.patch(
    "/feature-flags/{key}", response_model=FeatureFlag, dependencies=[Depends(require_developer)]
)
...
async def delete_feature_flag(
    key: str = Path(...), service: AdminService = Depends(get_admin_service)
):
...
async def list_settings(service: AdminService = Depends(get_admin_service)):
