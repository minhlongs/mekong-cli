...
async def get_cache_stats(current_user=Depends(get_current_active_superuser)):
...
async def invalidate_cache(
    request: InvalidationRequest, current_user=Depends(get_current_active_superuser)
):
...
async def browse_keys(
    ...
    return {"cursor": new_cursor, "keys": keys, "count": len(keys)}


@router.get("/keys/{key:path}")
async def get_key_details(key: str, current_user=Depends(get_current_active_superuser)):
