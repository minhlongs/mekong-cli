...
@router.get(
    "/", response_model=List[PromptResponse], dependencies=[Depends(get_current_active_superuser)]
)
...
@router.get(
    "/{slug}", response_model=PromptResponse, dependencies=[Depends(get_current_active_superuser)]
)
...
@router.post(
    "/", response_model=PromptResponse, dependencies=[Depends(get_current_active_superuser)]
)
...
def update_prompt(prompt_id: int, prompt_in: PromptUpdate, db: Session = Depends(get_db)):
    """
...
@router.delete(
    "/{prompt_id}", response_model=bool, dependencies=[Depends(get_current_active_superuser)]
)
