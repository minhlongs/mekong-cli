...
async def search(
    ...
    results = search_service.search(
        query=q, indexes=target_indexes, filters=filters, limit=limit, offset=offset
    )
...
async def top_queries(
    limit: int = 10, analytics_service: SearchAnalyticsService = Depends(get_search_analytics)
):
    """
...
async def reindex(index: str, indexer: SearchIndexer = Depends(get_search_indexer)):
    """
...
async def index_stats(index: str, search_service: SearchService = Depends(get_search_service)):
    """
