...
async def list_my_api_keys(
    current_user=Depends(get_current_user), service: ApiKeyService = Depends(get_api_key_service)
):
...
async def list_my_webhooks(current_user=Depends(get_current_user), db=Depends(get_db)):
...
async def delete_webhook(
    config_id: str, current_user=Depends(get_current_user), db=Depends(get_db)
):
    # Verify ownership via join or two steps
    # Step 1: Get config
    config_res = (
        db.table("webhook_configs").select("api_key_id").eq("id", config_id).single().execute()
    )
...
async def get_my_usage_stats(
    days: int = 30, current_user=Depends(get_current_user), db=Depends(get_db)
):
    # Get all keys
    keys = db.table("api_keys").select("id").eq("user_id", current_user["id"]).execute()
    key_ids = [k["id"] for k in keys.data]
