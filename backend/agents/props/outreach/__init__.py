...
class OutreachAgent(OutreachAgentBase):
    ...
    def send_pitch(self, pitch_id: str) -> Pitch:
        if pitch_id not in self.pitches: raise ValueError("Pitch not found")
        pitch = self.pitches[pitch_id]
        pitch.status = PitchStatus.SENT
        pitch.sent_at = datetime.now()
        if pitch.contact_id in self.contacts: self.contacts[pitch.contact_id].last_contacted = datetime.now()
        return pitch

    def get_stats(self) -> Dict:
        pitches = list(self.pitches.values())
        return {
            "total_contacts": len(self.contacts), "total_pitches": len(pitches),
