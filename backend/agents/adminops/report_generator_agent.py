...
class ReportGeneratorAgent:
    """
    Report Generator Agent - Tạo Báo cáo Tự động
...
""",
    ...
    def generate_weekly(
        ...
        actions_section = "\n".join([f"- [ ] {a}" for a in (action_items or ["No action items"])])
    ...
    def generate_monthly(
        self, title: str, metrics: Dict, summary: str = "", performance: str = "", focus: str = ""
    ) -> Report:
    ...
    def get_recent_reports(self, count: int = 5) -> List[Report]:
        ...
        return sorted(self.reports.values(), key=lambda r: r.created_at, reverse=True)[:count]
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "by_type": {
                rt.value: len([r for r in reports if r.report_type == rt]) for rt in ReportType
            },
            "this_month": len([r for r in reports if r.created_at.month == datetime.now().month]),
        }
...
if __name__ == "__main__":
    ...
    print("📊 Report Generator Agent Demo\n")
    ...
    weekly = agent.generate_weekly(
        ...
        action_items=["A/B test CTA buttons", "Review ad spend allocation"],
    )

    print(f"📄 Report: {weekly.title}")
    print(f"   Type: {weekly.report_type.value}")
    print(
        f"   Period: {weekly.period_start.strftime('%m/%d')} - {weekly.period_end.strftime('%m/%d')}"
    )
