...
class ActivityTrackerAgent(ActivityEngine):
    ...
    def get_goal_progress(self) -> Dict:
        ...
        return {
            k: {
                "current": getattr(stats, k if k != "talk_time" else "talk_time_mins"),
                "goal": g,
                "percent": (getattr(stats, k if k != "talk_time" else "talk_time_mins") / g * 100)
                if g
                else 0,
            }
            for k, g in self.DAILY_GOALS.items()
        }
