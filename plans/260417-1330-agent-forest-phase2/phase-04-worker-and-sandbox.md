...
- `agent_runner` runs in subprocess with ONLY env `AGENT_CORE_OUTPUTS`, `MEKONGD_URL`, `PATH` set — drops other env (especially `JWT_SECRET_KEY`, `ANTHROPIC_API_KEY` unless explicitly needed by mekongd).
