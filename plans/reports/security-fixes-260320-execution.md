...
| 02 | Fallback Secrets | COMPLETED | JWT_SECRET required, fail-fast on missing |
...
# Test: Running tests without JWT_SECRET correctly fails:
...
**Note:** Tests require `JWT_SECRET` environment variable to run (expected behavior after Phase 02):
...
**To run tests:**
```bash
export JWT_SECRET="test-secret-for-development-only"
python3 -m pytest tests/core/ -v
```
