# 🏯 Hiến Pháp Agency OS - WIN-WIN-WIN Architecture
...
- `antigravity/mcp_servers/workflow_server/` - Workflow automation
- `antigravity/mcp_servers/quota_server/` - API quota management
...
### 1. LUÔN LUÔN TẠO GIÁ TRỊ CHO CẢ 3 BÊN
...
- ❌ Bill hours mà không tạo outcome
- ❌ Lấy tiền mà startup không grow
- ❌ Equity mà không add value
- ❌ Tư vấn mà không execution support
- ❌ One-sided win (chỉ agency win)

### 4. LUÔN LUÔN LÀM NHƯ SAU

- ✅ Equity + Cash aligned với startup success
- ✅ Success fee tied to outcomes
- ✅ Retainer exchanged for real deliverables
- ✅ Anti-dilution protection for founders
- ✅ Shared exit celebration
...
- **Giọng điệu**: Technical + Strategic
- **Phong cách**: Binh Pháp wisdom + Modern startup
...
Trước khi kết thúc mỗi ngày làm việc:
...
## 🚀 AntigravityKit Integration Rules (BẮT BUỘC)

> Tiêu chuẩn đồng bộ với Antigravity IDE 2026

### 1. LUÔN LUÔN ánh xạ với AntigravityKit

```
Mọi phát triển mới PHẢI:
┌───────────────────────────────────────────────────┐
│  ✅ Check AntigravityKit modules trước            │
│  ✅ Sử dụng core modules (DNA, Magnet, Revenue)   │
│  ✅ Follow Binh Pháp architecture                 │
│  ✅ Update test_wow.py khi thêm feature           │
│  ✅ Sync với /antigravity-sync workflow           │
└───────────────────────────────────────────────────┘
```
...
antigravity/
├── core/
│   ├── agency_dna.py      # Identity + Vietnamese tones
│   ├── client_magnet.py   # Lead generation
│   ├── revenue_engine.py  # Revenue tracking
│   └── content_factory.py # Content production
├── platform/
│   └── data_moat.py       # Proprietary intelligence
├── franchise/
│   └── manager.py         # Territory network
├── vc/
│   └── metrics.py         # VC readiness
└── cli/
    └── __init__.py        # CLI commands
```

### 6. Antigravity IDE Best Practices

- **Agent-First**: Delegate complex tasks to AI agents
- **Planning Mode**: Use for architecture, refactoring
- **Fast Mode**: Use for quick fixes, tests
- **Artifacts**: Always generate task.md, implementation_plan.md
- **MCP Security**: Only use trusted servers
