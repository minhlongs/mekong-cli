...
2. **PROJECT/.claude/_integration/skills/\<skill\>/.env** - Project skill-specific
3. **PROJECT/.claude/_integration/skills/.env** - Project shared across skills
4. **PROJECT/.claude/.env** - Project global defaults
5. **~/.claude/_integration/skills/\<skill\>/.env** - User skill-specific
6. **~/.claude/_integration/skills/.env** - User shared across skills
...
python ~/.claude/_pipeline/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal

# With verbose output
python ~/.claude/_pipeline/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal --verbose

# Find all locations where variable is defined
python ~/.claude/_pipeline/scripts/resolve_env.py GEMINI_API_KEY --find-all

# Show hierarchy for a skill
python ~/.claude/_pipeline/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal

# Export format for shell sourcing
eval $(python ~/.claude/_pipeline/scripts/resolve_env.py GEMINI_API_KEY --export)
```
...
if not api_key:
    ...
    print("Run: python ~/.claude/_pipeline/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal")
    sys.exit(1)
...
python ~/.claude/_pipeline/scripts/resolve_env.py TEST_VAR --verbose
...
python ~/.claude/_pipeline/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal --find-all
```
