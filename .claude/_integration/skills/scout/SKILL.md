...
**Typically precedes:** `/mk:debug` (debug after scouting), `/mk:fix` (fix after locating code), `/mk:code-review` (scout edge cases before review)
**Related:** `/mk:debug` (investigate after scouting), `/mk:brainstorm` (explore after scouting)
