...
### View Mode
- `/mk:preview <file.md>` - View markdown file in novel-reader UI
- `/mk:preview <directory/>` - Browse directory contents
- `/mk:preview --stop` - Stop running server

### Generation Mode (Markdown)
- `/mk:preview --explain <topic>` - Generate visual explanation (ASCII + Mermaid + prose)
- `/mk:preview --slides <topic>` - Generate presentation slides (one concept per slide)
- `/mk:preview --diagram <topic>` - Generate focused diagram (ASCII + Mermaid)
- `/mk:preview --ascii <topic>` - Generate ASCII-only diagram (terminal-friendly)

### Generation Mode (HTML)
- `/mk:preview --html --explain <topic>` - Self-contained HTML explanation
- `/mk:preview --html --slides <topic>` - Magazine-quality HTML slide deck
- `/mk:preview --html --diagram <topic>` - HTML diagram with zoom controls
- `/mk:preview --html --diff [ref]` - Visual diff review
- `/mk:preview --html --plan-review [plan-file]` - Plan vs codebase comparison
- `/mk:preview --html --recap [timeframe]` - Project context snapshot
...
| Flag without topic | Ask user: "Please provide a topic: `/mk:preview --explain <topic>`" |
...
| Server startup failure | Check if port in use, try `/mk:preview --stop` first |
...
Use `/mk:mermaidjs-v11` skill for Mermaid syntax validation.
...
- For `--slides`: consider invoking `/mk:ui-ux-pro-max` for richer style selection
