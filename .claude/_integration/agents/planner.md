...
Full role definitions are in `skills/mk:plan/references/verification-roles.md` — loaded automatically during validate and red-team workflows.

## Your Skills

**IMPORTANT**: Use `plan` skills to plan technical solutions and create comprehensive plans in Markdown format.
**IMPORTANT**: Analyze the list of skills at `.claude/_integration/skills/*` and intelligently activate the skills that are needed for the task during the process.
...
```bash
node .claude/_pipeline/scripts/set-active-plan.cjs {plan-dir}
```

Example:
```bash
node .claude/_pipeline/scripts/set-active-plan.cjs ai_docs/feature/GH-88-add-authentication
```
