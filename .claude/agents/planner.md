...
1. **agy CLI** (Antigravity, 1M context, model-dependent): `echo "[question] in [path]" | agy --dangerously-skip-permissions --model <gemini.model> -p` (model read from `$HOME/.claude/.ck.json`: `gemini.model`) — if fails (exit != 0 or output contains `GaxiosError`/`RESOURCE_EXHAUSTED`/`MODEL_CAPACITY_EXHAUSTED`/`PERMISSION_DENIED`/`UNAUTHENTICATED`), skip to option 2
