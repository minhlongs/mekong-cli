 ...
 *
 * Blocks access to directories listed in the shipped $HOME/.claude/.ckignore baseline
 * plus an optional project-local $HOME/.claude/.ckignore override at the git root.
 ...
 *
 * Configuration:
 * - Edit $HOME/.claude/.ckignore to customize shipped baseline patterns
...
try {
  ...
  try {
    ...
    const claudeDir = path.dirname(__dirname); // Go up from hooks/ to $HOME/.claude/
