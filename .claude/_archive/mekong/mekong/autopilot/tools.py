...
def write_file(repo_root: Path, rel_path: str, body: str, cf_filter: bool = False) -> ToolResult:
    """Write a file with optional Cloudflare boundary check."""
    ...
    # Public boundary check (CLAUDE.md)
    ...
    # Cloudflare filter — khi active, chỉ cho phép Cloudflare-related paths
    if cf_filter:
        cf_allowed_patterns = (
            "wrangler.toml",
            "worker-config",
            ...
            "package.json" if "cloudflare" in rel_str.lower() or "worker" in rel_str.lower() else "",
        )
        if not any(rel_str.startswith(p) or p in rel_str.lower() for p in cf_allowed_patterns if p):
            return ToolResult(False, f"[CF-FILTER] REJECTED: {rel_path} không phải Cloudflare-related file. Chỉ cho phép: wrangler.toml, cloudflare/, workers/, config CF workers.")
...
def edit_file(repo_root: Path, rel_path: str, body: str, cf_filter: bool = False) -> ToolResult:
    ...
    # Cloudflare filter boundary check
    if cf_filter:
        rel_str = str(rel_path).lstrip("./")
        cf_allowed_patterns = (
            "wrangler.toml",
        ...
        if not any(rel_str.startswith(p) or p in rel_str.lower() for p in cf_allowed_patterns if p):
            return ToolResult(False, f"[CF-FILTER] REJECTED edit: {rel_path} không phải Cloudflare-related file.")
