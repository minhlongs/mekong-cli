...
function loadConfig(cwd) {
  // Read $HOME/.claude/.ck.json (canonical) or .ck.json (project root) scoped to cwd.
  let user = {};
...
function formatMessage(signals, breaches, severity) {
  ...
  return [
    ...
    `  Task(subagent_type="ck:code-simplifier", prompt="Simplify keeping behavior identical: <files>")`,
