 ...
 *
 * Run: node --test $HOME/.claude/hooks/tests/scout-block/scout-checker.test.cjs
 */
...
describe('checkScoutBlock - project-local $HOME/.claude/.ckignore discovery', () => {
  it('allows build paths via the git-root $HOME/.claude/.ckignore override', () => {
  ...
  it('still blocks node_modules and points to the git-root $HOME/.claude/.ckignore file', () => {
    const projectRoot = createProjectRoot({ rootPatterns: ['!build'] });
    const r = checkScoutBlock({
      toolName: 'Read',
  ...
  it('ignores nested $HOME/.claude/.ckignore files below the git root', () => {
