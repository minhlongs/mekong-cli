...
function buildSessionSection(staticEnv = {}) {
  ...
  return [
    ...
    `- Subagents: 200K context each. Spawn wisely. Include env info when delegating.`,
    ``
  ];
}
...
function buildModularizationSection() {
  return [
    `## Modularization`,
    `Check existing modules first. kebab-case (JS/TS/Python/shell). Split >200 LOC. Skip for md/txt/config files.`,
    ``
  ];
}
