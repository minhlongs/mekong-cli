...
```bash
node $HOME/.claude/skills/watzup/scripts/watzup-scan.cjs --json
```
...
When developing from this source repository before install, use `node claude/skills/watzup/scripts/watzup-scan.cjs --json` only if `$HOME/.claude/skills/...` is not present.
