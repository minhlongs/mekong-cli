---
name: mlx-inference
description: MLX LLM inference on Apple Silicon (M1 Max). Trigger when working with local LLM servers, model routing, or debugging inference issues. Contains the actual model IDs, ports, and performance characteristics of running mlx_lm.server instances. CRITICAL: Docker containers CANNOT access Metal GPU — LLMs must stay bare metal.
user-invocable: true
when_to_use: "Invoke when configuring local LLM inference on Apple Silicon, debugging mlx_lm.server, routing model requests, or when Metal GPU access is relevant."
category: dev-tools
keywords: [mlx, llm, inference, apple-silicon, metal, local-ai, model-routing]
---
...
- Apple Silicon does NOT support BF16 compute natively. MLX converts at runtime — slight overhead.
