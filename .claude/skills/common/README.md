...
3. **.claude/.env file**
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > $HOME/.claude/.env
   ```

4. **.claude/skills/.env file** (shared across all Gemini skills)
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > $HOME/.claude/skills/.env
   ```

5. **Skill directory `.env` file**
   ```bash
   echo 'GEMINI_API_KEY=your-api-key' > $HOME/.claude/skills/your-skill/.env
   ```
