...
- `--parallel`: Multi-agent execution with PEV DAG engine (dependency-aware parallel steps). The PEV engine decomposes the plan into a recipe of steps with dependencies, executes them in parallel using DAG scheduling, and validates each step with verification gates.
...
| Phase | Subagent / Engine | Requirement |
|-------|-------------------|-------------|
| Research | `researcher` | Optional in fast/code |
| Scout | `ck:scout` | Optional in code |
| Plan | `planner` | Optional in code |
| UI Work | `ui-ux-designer` | If frontend work |
| Parallel Execution | `pev-engine` | **MUST** invoke for `--parallel` mode with 3+ dependent steps. Decomposes plan into DAG recipe, runs up to 4 concurrent workers, validates each step with verification gates. Failed steps trigger self-healing (LLM-powered correction) before retry. |
| Testing | `tester`, `debugger` | **MUST** spawn |
| Review | `code-reviewer` | **MUST** spawn |
| Finalize | `/ck:project-management` skill + `docs-manager`, `git-manager` subagents | **MUST** invoke all |

**CRITICAL ENFORCEMENT:**
- Step 3 (parallel mode): **MUST** invoke `pev-engine` via `Skill(skill="pev-engine", args="<recipe>")` for DAG-based execution when plan has 3+ dependent steps
...
## PEV Engine Integration

When `--parallel` mode is active and the plan contains 3+ steps with dependencies, invoke the PEV engine skill (`pev-engine`) for DAG-based execution.

**How it works:**
1. **Decompose** — The PEV engine breaks the plan into a recipe of atomic steps, each with `id`, `title`, `mode`, `command`, `deps`, `verify`, and `retry` fields.
2. **Execute** — Steps with no dependencies run in parallel (up to 4 concurrent workers). Steps wait for their `deps` to complete before executing.
3. **Verify** — Each step runs verification gates (exit_code, file_exists, output_contains, etc.) after completion.
4. **Heal** — Failed steps trigger self-healing (LLM-powered correction) before retry. Persistent failures roll back completed dependent steps.

**Concurrency:** Up to 4 parallel workers by default. Override with `concurrency: N` in the recipe header.

**Retry policy:** Exponential backoff, max 3 attempts, full jitter. Exit code 2 (bad input) is non-retryable.

**Invocation:**
```
Skill(skill="pev-engine", args="<YAML recipe or plan path>")
```

**Protected flows:** PEV execution must not touch Setup Wizard, Telegram Bot webhooks, or Payment Flow IPN handlers. These are customer-facing and must remain stable.
