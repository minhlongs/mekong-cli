...
│    Uses Antigravity Proxy Adapter (port 11436) for LLM calls              │
└─────────────────────────────────────────────────────────────────┘
```
...
| Proxy | Antigravity Proxy Adapter (port **11436** → backends 9191+9192) |
...
```
CC CLI ──→ http://localhost:11436 ──→ Adapter ──→ 9191 (Billwill Ultra)
                                              └──→ 9192 (Cashback Ultra)
                                        │
                                        ├── Dual-account rotation (2 Ultra Gemini)
...
| `ADAPTER_PORT` | **11436** (🔒 CẤM THAY ĐỔI — xem ĐIỀU 56) |
| `BACKEND_PORTS` | 9191 (Billwill) + 9192 (Cashback) — CẤM connect trực tiếp |
...
## ĐIỀU 56 — ANTIGRAVITY PROXY ADAPTER PORT 11436 (🔒 KHÓA CỨNG)

> **CẤM QUÊN. CẤM THAY ĐỔI PORT. MỌI AGENT PHẢI ĐỌC.**

### Kiến trúc 3-Port (2026-02-20)

```
┌─────────────────────────────────────────────────────────────┐
│ CC CLI / Agent  →  http://localhost:11436  (ADAPTER)       │
│                         ↓ (auto-rotation)                  │
│                    ┌────┴────┐                              │
│                    ↓         ↓                              │
│               :9191        :9192                           │
...
```
🔒 ADAPTER PORT = 11436 — LOCKED PERMANENTLY
🔒 ANTHROPIC_BASE_URL = http://localhost:11436
🔒 settings.json: "ANTHROPIC_BASE_URL": "http://localhost:11436"
🔒 config.js: PROXY_PORT: 11436, CLOUD_BRAIN_URL: 'http://127.0.0.1:11436'

⛔ CẤM connect trực tiếp 9191 hoặc 9192 — bypass rotation!
⛔ 9191 + 9192 là BACKEND ports — chỉ Adapter mới được gọi chúng
```

### Lý do khóa cứng (2026-02-20):
- Port 11436 = Adapter control port, rotates giữa 2 Ultra accounts
- Port 9191 = Backend #1 (Billwill) — CẤM connect trực tiếp!
- Port 9192 = Backend #2 (Cashback) — CẤM connect trực tiếp!
- Connect 9191 trực tiếp = bypass rotation → chỉ dùng 1 account → rate limit nhanh
- Đã mất nhiều phiên debug vì agent đọc CLAUDE.md cũ với port 9191

### Troubleshooting ConnectionRefused:
1. Check adapter alive: `curl http://localhost:11436/health`
2. Check backends: `curl http://localhost:9191/health` + `curl http://localhost:9192/health`
3. Check settings.json: `grep ANTHROPIC_BASE_URL ~/.claude/settings.json` → phải là 11436
4. Restart adapter: `bash apps/openclaw-worker/scripts/proxy-lan-start.sh`
5. Restart CC CLI AFTER adapter is alive
