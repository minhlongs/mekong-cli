...
function spawnMission(prompt, projectDir, timeoutMs, opts = {}) {
	...
	return new Promise((resolve) => {
		...
		log(
			`SPAWN #${num}${isRetry ? ' (RETRY)' : ''}: claude -p [model=${model}] [cwd=${projectDir}] [timeout=${Math.round(timeoutMs / 60000)}min]`,
		);

		const child = spawn('claude', args, {
			cwd: projectDir,
			stdio: ['ignore', 'pipe', 'pipe'], // stdin MUST be ignore
			env: { ...process.env, ANTHROPIC_BASE_URL: `http://127.0.0.1:${config.PROXY_PORT}` },
			timeout: timeoutMs,
		});
		...
		child.stdout.on('data', (chunk) => {
			...
			if (last && last.length > 5) {
				...
				try {
					fs.appendFileSync(config.LOG_FILE, `[${new Date().toISOString().slice(11, 19)}] [mission-${num}] ${truncated}\n`);
				} catch (e) {}
			}
		});
