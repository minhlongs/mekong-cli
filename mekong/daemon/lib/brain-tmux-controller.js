...
function getActivePaneProjects(sessionName = config.TMUX_SESSION) {
	const paneMap = {};
	try {
		const raw = execSync(`tmux list-panes -t ${sessionName}:0 -F "#{pane_index}:#{pane_current_path}" 2>/dev/null`, {
			encoding: 'utf-8',
			timeout: 5000,
		}).trim();
		...
		log(
			`[DYNAMIC ROUTING] Discovered ${Object.keys(paneMap).length} panes: ${Object.entries(paneMap)
