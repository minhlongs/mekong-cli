...
// CTO uses ALL 4 DashScope keys with auto-rotation
// Pool 1: OpenAI-compatible endpoint (Coding Plan sk-sp-* keys + CTO key)
// Pool 2: Anthropic-compatible endpoint (text-model keys sk-4d2*, sk-562*)
const CTO_PROVIDERS = [
	{
		id: 'dashscope-openai',
		...
		keys: [
			process.env.CTO_DASHSCOPE_KEY || 'sk-80d8537485d04f609c498f1881e67c6f', // Key C — CTO
			...
			'sk-4d2965a589ca4d9da2ea05e4bd200d97', // Key D (Text Model)
			'sk-562c92be8e0f4670a7147699d65d4905', // Key E (Text Model)
		],
		model: 'qwen3.5-plus',
	},
	{
...
function getActiveProvider() {
	return CTO_PROVIDERS[_providerIdx];
}
function getActiveKey() {
	return getActiveProvider().keys[_keyIdx];
}
function getActiveUrl() {
	return getActiveProvider().url;
}
function getActiveModel() {
	return process.env.CTO_LLM_MODEL || getActiveProvider().model;
}
		...
		const attemptOllamaFallback = () => {
			...
			const ollamaPayload = JSON.stringify({
				...
				messages: [
					...
					{
						role: 'user',
						content: `Pane: P${paneIdx} | Project: ${projectName}\n\n--- TERMINAL OUTPUT ---\n${trimmedOutput}\n--- END ---`,
					},
				],
				stream: false,
			});
...
function guardCheck(paneOutput, regexState, projectName, paneIdx) {
	return new Promise((resolve, reject) => {
		...
		req.on('error', () => {
			rotateKey();
			resolve({ agree: true, correctedState: regexState, reason: 'network fail', shouldAct: true });
		});
		req.on('timeout', () => {
			req.destroy();
			rotateKey();
			resolve({ agree: true, correctedState: regexState, reason: 'timeout', shouldAct: true });
		});
		req.write(payload);
		req.end();
	});
