...
const CUU_DIA = {
	...
	tranh: {
		name: '爭地 Tranh Địa',
		situation: 'Staging deploy',
		skills: ['web-testing', 'devops', 'frontend-development', 'backend-development'],
	},
	giao: { name: '交地 Giao Địa', situation: 'API contracts', skills: ['databases', 'mcp-builder', 'mcp-management', 'better-auth'] },
	cu: {
		name: '衢地 Cù Địa',
		situation: 'Multi-platform',
		skills: ['shopify', 'payment-integration', 'mobile-development', 'google-adk-python'],
	},
	trong: {
		name: '重地 Trọng Địa',
		situation: 'Production deep',
		skills: ['debug', 'problem-solving', 'context-engineering', 'chrome-devtools'],
	},
...
function getCommandAnnotation(cmd) {
	...
	return COMMANDS[normalized] || Object.entries(COMMANDS).find(([key]) => normalized.startsWith(key.toLowerCase()))?.[1] || null;
}
