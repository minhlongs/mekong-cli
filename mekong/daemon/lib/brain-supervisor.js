...
function capturePaneOutput() {
	...
	for (let paneIdx = 0; paneIdx < 2; paneIdx++) {
		try {
			output +=
				execSync(`tmux capture-pane -t ${TMUX_SESSION}.${paneIdx} -p -S -30 2>/dev/null`, { encoding: 'utf-8', timeout: 5000 }) + '\n';
		} catch (e) {
	...
	try {
		...
		if (!isBusy(output) && !isAtPrompt(output)) {
			...
			if (!stuckSince) {
				...
				try {
					heartbeatStale = require('./brain-heartbeat').isBrainHeartbeatStale();
				} catch (e) {}
				if (heartbeatStale) {
