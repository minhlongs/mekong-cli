...
const PROXY_URL = process.env.ANTHROPIC_BASE_URL || 'https://dashscope-intl.aliyuncs.com/compatible-mode/v1';
...
async function analyzeProjectHealth(project, projectDir) {
	...
	try {
		const todoOutput = execSync(`grep -rE "TODO|FIXME" . --exclude-dir={node_modules,dist,.next,.claude} 2>/dev/null | wc -l`, {
			cwd: projectDir,
		})
			.toString()
			.trim();
		...
		const consoleLogs = execSync(`grep -r "console.log" . --exclude-dir={node_modules,dist,.next,.claude} 2>/dev/null | wc -l`, {
			cwd: projectDir,
		})
			.toString()
			.trim();
