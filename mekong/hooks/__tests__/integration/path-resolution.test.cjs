 ...
 * Run: node --test $HOME/.claude/hooks/__tests__/integration/path-resolution.test.cjs
 *
...
describe('Issue #327: Path Resolution Integration', () => {
  ...
  describe('Subdirectory Workflow', () => {
    ...
    it('all hooks use CWD as base when in subdirectory', async () => {
      ...
      assert.ok(
        subagentContext.includes(subdirPath) || subagentContext.includes('$HOME/.claude/hooks'),
  ...
  describe('Monorepo/Worktree Scenarios', () => {
    ...
    it('scout-block uses payload.cwd to discover a git-root $HOME/.claude/.ckignore', async () => {
