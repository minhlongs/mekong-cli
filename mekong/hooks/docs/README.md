...
```bash
cp $HOME/.claude/hooks/notifications/.env.example ~/.claude/.env
```
...
Add to your `$HOME/.claude/settings.json` or `$HOME/.claude/settings.local.json`:
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "node $HOME/.claude/hooks/notifications/notify.cjs"
          }
        ]
      }
    ],
...
```bash
echo '{"hook_event_name":"Stop","cwd":"'$(pwd)'","session_id":"test123"}' | node $HOME/.claude/hooks/notifications/notify.cjs
```
...
- **Env Cascade**: `process.env` > `~/.claude/.env` > `$HOME/.claude/.env`
...
**Blocked Patterns** (baseline in `$HOME/.claude/.ckignore`, optional override in `<project-root>/.claude/.ckignore`):
- `node_modules`, `__pycache__`, `.git`, `dist`, `build`

**Testing:**
```bash
echo '{"tool_input":{"command":"ls node_modules"}}' | node $HOME/.claude/hooks/scout-block.cjs
```
...
```bash
node $HOME/.claude/hooks/workflow-artifact-gate.cjs --stage finalize --artifact-dir plans/example/reports/harness
```

Opt in through `$HOME/.claude/.ck.json`:
...
Then add hook commands to `$HOME/.claude/settings.json`:
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node \"$HOME/.claude/hooks/workflow-artifact-gate.cjs\""
          }
        ]
      }
    ],
...
- Verify hooks are added to `$HOME/.claude/settings.json`
