...
# Cloudflare-focused system prompt
CF_SYSTEM_PROMPT = """You are Mekong /idea autopilot — Cloudflare specialist.

You have a 1M-token context window (DeepSeek). You drive a Plan → Execute → Verify → Reflect loop to build Cloudflare-optimized solutions.

FOCUS AREAS (Cloudflare):
- Workers (JavaScript/TypeScript, WASM)
...
- Performance optimization (caching, edge logic)
...
NEVER write prose outside these blocks. Never ask the user a question — you're autonomous.

CONSTRAINTS (CF-FILTER MODE)
- ONLY create/modify Cloudflare-related files:
  * wrangler.toml / worker configuration
  * src/workers/ or workers/ directories
  * cloudflare/ or .cloudflare/ directories
  * package.json với Cloudflare dependencies (wrangler, @cloudflare/workers)
  * vitest configs for Workers testing
  * README với deployment instructions
- Refuse writes to non-Cloudflare paths (apps/, mekong/daemon/, etc.)
...
- Stay focused on Cloudflare optimization. Don't yak-shave.

THE WORKFLOW (default cadence)
1. PLAN  — break the idea into ≤6 Cloudflare tasks. WRITE plan to `.mekong/idea/<run>/plan.md`.
2. For each task:
   a. READ relevant files
   b. WRITE / EDIT (Cloudflare files only)
   c. TYPECHECK (if TypeScript)
   d. TEST (wrangler dev / vitest)
   e. CHECKPOINT
3. When all tasks complete + tests green → DONE.
"""


# Default system prompt (non-CF)
DEFAULT_SYSTEM_PROMPT = """You are Mekong /idea autopilot — an autonomous senior engineer.
...
class RunOptions:
    ...
    cf_filter: bool = False        # Cloudflare-focused mode
...
class IdeaLoop:
    ...
    def _dispatch(self, call: ToolCall) -> tuple[ToolResult, bool]:
        ...
        if t == "WRITE":
            r = write_file(REPO_ROOT, call.args, call.body, cf_filter=self.opts.cf_filter)
            if r.ok:
                self.files_touched.add(call.args)
            return r, False
        if t == "EDIT":
            r = edit_file(REPO_ROOT, call.args, call.body, cf_filter=self.opts.cf_filter)
            if r.ok:
                self.files_touched.add(call.args)
            return r, False
        if t == "BASH":
    ...
    def _bootstrap_context(self) -> None:
        ...
        # Chọn system prompt dựa trên cf_filter
        system_prompt = CF_SYSTEM_PROMPT if self.opts.cf_filter else DEFAULT_SYSTEM_PROMPT
        ...
        if self.opts.cf_filter:
            self.ctx.add("user", "[CF-FILTER MODE] Chỉ generate và modify Cloudflare-related files (Workers, wrangler.toml, Pages, D1, KV, R2, Durable Objects).")
    ...
    def run(self) -> RunResult:
        ...
        (self.run_dir / "result.json").write_text(json.dumps(asdict(run_result), default=str, indent=2), encoding="utf-8")
        self._audit("end", asdict(run_result))
        return run_result
