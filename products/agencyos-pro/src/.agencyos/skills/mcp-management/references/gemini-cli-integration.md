...
Gemini CLI reads MCP servers from `.gemini/settings.json`. Create a symlink to `.agencyos/.mcp.json`:
...
ln -sf .agencyos/.mcp.json .gemini/settings.json

# Create symlink (Windows - requires admin or developer mode)
mklink .gemini\settings.json .agencyos\.mcp.json
```
...
1. **Configuration Loading**: Reads `.gemini/settings.json` (symlinked to `.agencyos/.mcp.json`)
...
Should show symlink pointing to `.agencyos/.mcp.json`.
