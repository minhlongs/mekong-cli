...
def find_api_key(skill_dir: Optional[Path] = None) -> Optional[str]:
    ...
    # Step 3: Check ./.agencyos/.env
    claude_env = project_dir / '.agencyos' / '.env'
    ...
    # Step 4: Check ./.agencyos/skills/.env
    claude_skills_env = project_dir / '.agencyos' / 'skills' / '.env'
...
def find_env_var(var_name: str, skill_dir: Optional[Path] = None) -> Optional[str]:
    ...
    env_files = [
        project_dir / '.env',
        project_dir / '.agencyos' / '.env',
        project_dir / '.agencyos' / 'skills' / '.env',
        skill_dir / '.env'
    ]
...
def get_api_key_or_exit(skill_dir: Optional[Path] = None) -> str:
    ...
    if not api_key:
        ...
        print("\n3️⃣  .agencyos/.env file:", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {project_dir}/.agencyos/.env", file=sys.stderr)

        print("\n4️⃣  .agencyos/skills/.env file (shared across all Gemini skills):", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {project_dir}/.agencyos/skills/.env", file=sys.stderr)
