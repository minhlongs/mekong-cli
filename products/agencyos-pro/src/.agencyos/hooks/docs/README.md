...
**Configuration:** Already enabled in `.agencyos/settings.json`

**Default Blocked Patterns** (configured in `.agencyos/.ckignore`):
...
**Customization:**
Edit `.agencyos/.ckignore` to customize blocked patterns:
```bash
...
node .agencyos/hooks/tests/test-ckignore.js
```
...
echo '{"tool_input":{"file_path":"/tmp/test-large.js"}}' | node .agencyos/hooks/modularization-hook.js

# Test with small file (should be silent)
echo "console.log('test');" > /tmp/test-small.js
echo '{"tool_input":{"file_path":"/tmp/test-small.js"}}' | node .agencyos/hooks/modularization-hook.js
```
...
**Quick Test:**
```bash
echo '{"hookType":"Stop","projectDir":"'$(pwd)'","sessionId":"test","toolsUsed":[{"tool":"Read","parameters":{"file_path":"test.ts"}}]}' | ./.agencyos/hooks/discord_notify.sh
```
...
**Quick Test:**
```bash
echo '{"hookType":"Stop","projectDir":"'$(pwd)'","sessionId":"test","toolsUsed":[]}' | ./.agencyos/hooks/telegram_notify.sh
```
...
**Usage:**
```bash
./.agencyos/hooks/send-discord.sh 'Your message here'
```
...
2. **.agencyos/.env** - Project-level Claude configuration
3. **.agencyos/hooks/.env** - Hook-specific configuration
...
**OR `.agencyos/.env`** (for project-specific overrides):
```bash
# Same variables as above
```

**OR `.agencyos/hooks/.env`** (for hook-only configuration):
```bash
# Same variables as above
```
...
Hooks are configured in `.agencyos/settings.local.json`:
...
- Verify `.agencyos/settings.json` uses `node .agencyos/hooks/scout-block.js`
- Test manually: `echo '{"tool_input":{"command":"ls node_modules"}}' | node .agencyos/hooks/scout-block.js`
...
- Review [Claude Code Documentation](https://docs.agencyos.com/claude-code)
- Report issues at [Claude Code GitHub](https://github.com/anthropics/claude-code/issues)

## Additional Resources

- [Claude Code Documentation](https://docs.agencyos.com/claude-code)
- [Claude Code Hooks Reference](https://docs.agencyos.com/claude-code/hooks)
