...
export default function TreasuryPage() {
  ...
  if (!treasury) return <div className="p-8 text-center text-gray-500 dark:text-gray-400">Loading treasury...</div>

  return (
    <div className="p-8 max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold mb-8 text-gray-900 dark:text-gray-100">💰 Community Treasury</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-6 shadow border border-gray-100 dark:border-gray-700">
          <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400">Total Inflow</h3>
          <p className="text-2xl font-bold mt-2 text-green-600 dark:text-green-400">+${treasury.total_in.toLocaleString()}</p>
        </div>
        <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-6 shadow border border-gray-100 dark:border-gray-700">
          <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400">Total Outflow</h3>
          <p className="text-2xl font-bold mt-2 text-red-500 dark:text-red-400">-${treasury.total_out.toLocaleString()}</p>
        </div>
      </div>

      <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-4">Recent Transactions</h2>
      <div className="bg-gray-50 dark:bg-gray-800 rounded-xl shadow border border-gray-100 dark:border-gray-700 overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {treasury.recent_transactions.map(t => (
              <tr key={t.id} className="hover:bg-gray-100 dark:hover:bg-gray-750">
                <td className="px-6 py-4 text-gray-900 dark:text-gray-100">{t.description}</td>
                <td className={`px-6 py-4 font-medium ${t.amount >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>
      ...
      <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-sm border border-blue-100 dark:border-blue-800">
        <p className="font-medium text-blue-900 dark:text-blue-200 mb-1">💡 Treasury Governance</p>
