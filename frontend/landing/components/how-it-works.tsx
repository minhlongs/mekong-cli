const STEPS = [
  {
    ...
    accent: 'border-[var(--md-outline-variant)] bg-[var(--md-surface-container-low)]',
  },
  {
    number: '02',
    title: 'DAG executes',
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {STEPS.map((step, i) => (
            <div
              key={step.number}
              className={`relative rounded-xl border p-6 transition-all duration-300 hover:-translate-y-1 ${step.accent}`}
            >
              ...
              <div className="mb-5 flex items-center gap-3">
                ...
                {i < STEPS.length - 1 && (
                  <div className="hidden h-px flex-1 bg-gradient-to-r from-[var(--md-outline-variant)] to-transparent lg:block" />
                )}
              </div>

              {/* Step label */}
              <div className="mb-1.5 font-mono text-xs font-semibold tracking-widest text-[var(--md-on-surface-variant)]">
                STEP {step.number}
              </div>

              <h3 className="mb-2 font-semibold text-[var(--md-on-surface)]">{step.title}</h3>
              <p className="text-sm leading-relaxed text-[var(--md-on-surface-variant)]">{step.desc}</p>
            </div>
          ))}
        </div>

        {/* Bottom callout */}
        <div className="mt-10 rounded-xl border border-[var(--md-outline-variant)] bg-[var(--md-surface-container-low)] px-6 py-4 text-center">
          <p className="text-sm text-[var(--md-on-surface-variant)]">
            <span className="font-mono text-[var(--md-primary)]">Plan → Execute → Verify</span>
