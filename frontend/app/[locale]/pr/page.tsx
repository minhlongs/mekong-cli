...
export default function PRPage({ params: { locale } }: { params: { locale: string } }) {
    ...
    return (
        <MD3AppShell title="PR Hub 📰" subtitle="Media • Coverage • Reputation • Relations">
                ...
                supportingContent={
                    <MD3Card headline="Quick Actions" subhead="PR Tools">
                        <div className="space-y-2">
                            {[{ icon: '📰', label: 'Releases' }, { icon: '📊', label: 'Coverage' }, { icon: '👥', label: 'Contacts' }, { icon: '💬', label: 'Outreach' }, { icon: '📋', label: 'Reports' }, { icon: '⚙️', label: 'Settings' }].map((action) => (
