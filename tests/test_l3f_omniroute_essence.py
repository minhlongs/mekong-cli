...
def test_fallback_chain_order_and_banned_guard():
    ...
    # đủ model theo spec
    assert sum(len(v) for v in MODEL_FAMILIES.values()) >= 10
...
def test_chat_locked_model_uses_family_sibling():
    with tempfile.TemporaryDirectory() as td:
        ...
        client.chat("claude-fable-5", [{"role": "user", "content": "x"}])
        assert called[0] == "claude-opus-5"  # sibling đầu tiên chưa locked
...
def test_chat_pin_success_records_last_model():
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            assert called == ["claude-opus-5"]  # pin thắng forced
            assert store.get(s.id).last_model == "claude-opus-5"
        finally:
...
def test_health_per_model_with_lockout():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            assert len(results) == 3                      # fable + sonnet + opus
