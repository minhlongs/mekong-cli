...
- MEKONG_JWT_SECRET not set, legacy not set → 503
...
JWT_SECRET = "test-jwt-secret-32-bytes-padding!!"  # >= 32 chars
...
# ---------------------------------------------------------------------------
# 10. No MEKONG_JWT_SECRET (and no MEKONG_ADMIN_TOKEN) → 503
# ---------------------------------------------------------------------------
...
class TestBothSecretsAbsent:
    def test_503_when_both_missing(
        ...
        """If neither MEKONG_ADMIN_TOKEN nor MEKONG_JWT_SECRET is set → 503."""
        monkeypatch.setattr(vpr, "CONFIG_DIR", tmp_path)
