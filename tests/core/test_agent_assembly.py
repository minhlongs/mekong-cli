...
class TestBuildSystemPrompt:
    def test_prompt_includes_role(self):
        ...
        registry = str(Path(__file__).resolve().parents[2] / "src" / "harness" / "agents" / "config" / "registry.yaml")
        factory_instance = AgentFactory(config_path=registry)
        with patch("harness.agents.assembly.AgentFactory", return_value=factory_instance):
