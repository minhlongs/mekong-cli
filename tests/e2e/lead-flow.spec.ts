...
test.describe('Luồng đăng ký tư vấn Nhịp Điệu Xanh', () => {
  ...
  test('Người dùng đăng ký nhận tư vấn thành công với thông tin đầy đủ', async ({ page }) => {
    ...
    const successHeading = page.locator('div[role="status"] h3')
    await expect(successHeading).toContainText('Đăng Ký Thành Công!')

    const successMessage = page.locator('div[role="status"] p')
    await expect(successMessage).toContainText('đội ngũ chuyên viên Nhịp Điệu Xanh sẽ liên hệ', { ignoreCase: true })
  })
  ...
  test('Hiển thị thông báo lỗi khi không nhập số điện thoại', async ({ page }) => {
    ...
    const errorMessage = page.locator('div[role="alert"]:has-text("Vui lòng nhập Số điện thoại")')
  ...
  test('Hiển thị thông báo lỗi khi nhập số điện thoại sai định dạng', async ({ page }) => {
    ...
    const errorMessage = page.locator('div[role="alert"]:has-text("Số điện thoại không đúng định dạng")')
    await expect(errorMessage).toBeVisible()
  })
})
