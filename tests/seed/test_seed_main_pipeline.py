...
def _run_with_mocks(task: str, llm_responses: list[str] | None = None):
    ...
    # Mock factory returning agents that parse LLM responses
    ...
    def _make_mock_agent(name: str):
        ...
        if name == "ceo":
            agent.create_plan.side_effect = lambda task: _next_response()
        elif name == "eng":
            agent.execute_plan.side_effect = lambda plan: [_next_response().get("text", "")]
        elif name == "tester":
            agent.verify.side_effect = lambda task, outputs: _next_response()
        return agent

    mock_factory = MagicMock()
    mock_factory.create.side_effect = lambda aid, **kw: _make_mock_agent(aid)

    with tempfile.TemporaryDirectory() as tmp_outputs:
        with (
            patch("seed.main.get_llm_client", return_value=mock_llm),
            patch("seed.main._load_factory", return_value=mock_factory),
