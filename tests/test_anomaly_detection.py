...
class TestUsageMetering:
    ...
    def metering(self, tmp_path: Path) -> UsageMetering:
        """Create metering with temp db file."""
        db_path = tmp_path / "test_metering.db"
        return UsageMetering(db_path=db_path)
    ...
    @pytest.mark.skip(reason="UsageTracker has no subscribe/event mechanism")
