...
async def mcu_deduct(
    ...
    idempotency_key = (
        ...
        or f"{request.tenant_id}:{request.mission_id or 'direct'}:{request.complexity}"
    )
    ...
    try:
        ...
        logger.error(
            "MCU deduct failed for tenant %s: %s", request.tenant_id, str(e)
        )
        raise HTTPException(
            status_code=500,
            detail={
