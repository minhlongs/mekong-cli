...
@contextmanager
def locked_read(path: Union[str, Path]):
    """Open a file in read-only mode with a shared advisory lock.
...
@contextmanager
def locked_read_write(path: Union[str, Path]):
    """Open a file in read-write mode with an exclusive advisory lock.
