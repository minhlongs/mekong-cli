"""Telegram bot AGI handlers — /agi command and sub-command dispatch."""
...
class AgiHandlers:
    """Mixin providing AGI loop command handlers."""
    ...
    async def agi_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if sub == "start":
            await self._agi_start(update)
        elif sub == "stop":
            await self._agi_stop(update)
        elif sub == "status":
            await self._agi_status(update)
        elif sub == "history":
            await self._agi_history(update)
        elif sub == "config":
            await self._agi_config(update)
        else:
            await update.message.reply_text(
                "♾️ Usage: /agi <start|stop|status|history|config>",
            )

    async def _agi_start(self, update: Update) -> None:
        """Start the AGI loop."""
        try:
    ...
    async def _agi_stop(self, update: Update) -> None:
        """Stop the AGI loop."""
        try:
            from src.core.agi_loop import get_agi_loop
    ...
    async def _agi_status(self, update: Update) -> None:
        """Show AGI loop status."""
        try:
            from src.core.agi_loop import get_agi_loop
    ...
    async def _agi_history(self, update: Update) -> None:
        """Show AGI improvement history."""
        try:
            from src.core.agi_loop import get_agi_loop
    ...
    async def _agi_config(self, update: Update) -> None:
        """Show AGI configuration."""
        try:
            from src.core.agi_loop import get_agi_loop
