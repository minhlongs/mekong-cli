...
class CommandSanitizer:
  ...
  def sanitize(self, command: str) -> SanitizationResult:
    ...
    # --- Finding #9: Detect command chaining before tokenization ---
    # Reject input containing ; && || | or literal newlines
    # These characters allow chaining arbitrary commands regardless of
    # individual command safety.
    if _has_command_chaining(command):
      result.is_safe = False
      result.blocked_patterns.append("command_chaining")
    ...
    for pattern_str in self.deny_patterns:
      try:
        if re.search(pattern_str, command, re.IGNORECASE):
          ...
          result.blocked_reason = (
            f"Matches custom deny pattern: {pattern_str}"
          )
      except re.error:
        pass
    ...
    # --- Finding #16 fix: allow patterns never override blocks ---
    # Allow patterns are informational only when the command is already safe.
    # Blocks (deny + dangerous) always take precedence.
    if result.is_safe:
    ...
    # Set blocked reason if not safe (blocks always win over allow)
