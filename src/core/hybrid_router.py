...
async def route_and_execute(
    ...
    # Viral watermark — free tier outputs carry distribution link
    # Paid tiers (growth, pro) can opt out via tenant config
    final_output = exec_result.output
    watermark_enabled = True
    try:
        from src.raas.credits import CreditStore
        balance = CreditStore().get_balance(tenant_id)
        # Skip watermark for tenants with >200 credits (Growth+ tier)
        if balance > 200:
            watermark_enabled = False
    except Exception:
        pass
    if watermark_enabled and final_output and not final_output.endswith("mekongmind.pages.dev"):
        final_output += "\n\n---\n*Generated by [MekongMind](https://mekongmind.pages.dev) — 290 commands, runs locally via Ollama*"
