...
class EnhancedControlCenter:
    ...
    def _load_initial_config(self):
        ...
        logger.info(
            f"Loaded {len(self.feature_flags)} feature flags and {len(self.remote_configs)} remote configs"
        )
            ...
            try:
                ...
                if self.redis_available:
                    ...
                    self.redis_client.setex(
                        analytics_key, 86400, json.dumps(report)
                    )  # 24 hours TTL
    ...
    def get_feature_analytics(self, flag_name: str = None) -> Dict[str, Any]:
        ...
        if flag_name:
            ...
            flag_usage = [
                u for u in self.analytics.usage_history if u.get("flag_name") == flag_name
            ]
            flag_errors = [
                e
                for e in self.analytics.usage_history
                if e.get("flag_name") == flag_name and not e.get("success", True)
            ]
            ...
            return {
                ...
                "performance_metrics": {
                    "avg_response_time": sum(
                        u.get("performance", {}).get("response_time", 0) for u in flag_usage
                    )
                    / max(len(flag_usage), 1),
            ...
            return {
                ...
                "enabled_flags": len(
                    [f for f in self.feature_flags.values() if f.get("enabled", False)]
                ),
                "usage_counts": dict(self.analytics.usage_counts),
                "user_segments": {
                    name: len(users) for name, users in self.analytics.user_segments.items()
                },
    ...
    def _calculate_performance_summary(self) -> Dict[str, Any]:
        ...
        response_times = [
            u.get("performance", {}).get("response_time", 0)
            for u in self.analytics.usage_history[-1000:]
        ]
        error_count = len(
            [u for u in self.analytics.usage_history[-1000:] if not u.get("success", True)]
        )
    ...
    def _cleanup_old_analytics(self):
        ...
        self.analytics.usage_history = deque(
            [u for u in self.analytics.usage_history if u["timestamp"] > cutoff_time], maxlen=10000
        )
...
def set_feature_flag(
    ...
    """Set feature flag with advanced targeting."""
    enhanced_control_center.set_feature_flag(
        flag_name, enabled, config, user_segment, rollout_percentage
    )
