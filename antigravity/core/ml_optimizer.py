...
class MLOptimizer:
    ...
    def _calculate_statistical_optimization(
        self, base_price: float, features: Dict[str, Any]
    ) -> MLOptimizationResult:
        ...
        if "ensemble" in self.models:
            try:
                ...
                y = np.array(
                    [features.get("conversion_rate", 0.1)]
                )  # Use historical conversion rate
...
def calculate_ai_optimized_price(
    base_price: float, features: Dict[str, Any], context: Dict[str, Any] = None
) -> MLOptimizationResult:
