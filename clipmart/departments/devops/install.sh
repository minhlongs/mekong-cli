#!/usr/bin/env bash
# Mekong CLI — Install dept-devops
set -euo pipefail
...
dry_run() {
  ...
  echo "Commands available: devops-deploy-pipeline,devops-rollback,deploy"
  echo "Skills available: devops,devops-agent"
  echo 'Pricing: $4.00 per infra_operation (floor $49/mo)'
  echo "Config file: $ENV_FILE"
  echo ""
  echo "To start: source $MEKONG_HOME/scripts/shell-init.sh"
  echo "First command: mekong $(echo 'devops-deploy-pipeline,devops-rollback,deploy' | cut -d',' -f1)"
...
main() {
  ...
  if [ "$mode" = "--dry-run" ]; then
    ...
    echo "  mekong $(echo 'devops-deploy-pipeline,devops-rollback,deploy' | cut -d',' -f1)"
  fi
}
