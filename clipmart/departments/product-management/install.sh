#!/usr/bin/env bash
# Mekong CLI — Install dept-product-management
set -euo pipefail

DEPT_ID="dept-product-management"
...
dry_run() {
  ...
  echo "Commands available: product-sprint-plan,product-discovery,product-launch-feature"
  echo "Skills available: product-manager-agent"
  echo 'Pricing: $10.00 per product_artifact (floor $49/mo)'
  echo "Config file: $ENV_FILE"
  echo ""
  echo "To start: source $MEKONG_HOME/scripts/shell-init.sh"
  echo "First command: mekong $(echo 'product-sprint-plan,product-discovery,product-launch-feature' | cut -d',' -f1)"
...
main() {
  ...
  if [ "$mode" = "--dry-run" ]; then
    ...
    echo "  mekong $(echo 'product-sprint-plan,product-discovery,product-launch-feature' | cut -d',' -f1)"
  fi
}
