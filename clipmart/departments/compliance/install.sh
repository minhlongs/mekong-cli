#!/usr/bin/env bash
# Mekong CLI — Install dept-compliance
set -euo pipefail

DEPT_ID="dept-compliance"
...
dry_run() {
  ...
  echo "Commands available: compliance-check,compliance-monitor,compliance-soc2-prep,audit-compliance"
  echo "Skills available: compliance-automation,regtech-compliance"
  echo 'Pricing: $5.00 per compliance_control (floor $49/mo)'
  echo "Config file: $ENV_FILE"
  echo ""
  echo "To start: source $MEKONG_HOME/scripts/shell-init.sh"
  echo "First command: mekong $(echo 'compliance-check,compliance-monitor,compliance-soc2-prep,audit-compliance' | cut -d',' -f1)"
...
main() {
  ...
  if [ "$mode" = "--dry-run" ]; then
    ...
    echo "  mekong $(echo 'compliance-check,compliance-monitor,compliance-soc2-prep,audit-compliance' | cut -d',' -f1)"
  fi
}
