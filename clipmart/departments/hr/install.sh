...
dry_run() {
  ...
  echo "Commands available: hr-onboard,hr-performance-cycle,hr-policy,hr-review"
  echo "Skills available: hr-agent,hr-people-ops"
  echo 'Pricing: $3.00 per hr_action (floor $49/mo)'
  echo "Config file: $ENV_FILE"
  echo ""
  echo "To start: source $MEKONG_HOME/scripts/shell-init.sh"
  echo "First command: mekong $(echo 'hr-onboard,hr-performance-cycle,hr-policy,hr-review' | cut -d',' -f1)"
...
main() {
  ...
  if [ "$mode" = "--dry-run" ]; then
    ...
    echo "  mekong $(echo 'hr-onboard,hr-performance-cycle,hr-policy,hr-review' | cut -d',' -f1)"
  fi
}
