#!/usr/bin/env bash
# Mekong CLI — Install dept-strategy
set -euo pipefail

DEPT_ID="dept-strategy"
DEPT_NAME="strategy"
...
dry_run() {
  ...
  echo "Commands available: okr,swot,board-report,annual,ipo-readiness-check"
  echo "Skills available: launch-strategy,pricing-strategy"
  echo 'Pricing: $15.00 per strategic_deliverable (floor $49/mo)'
  echo "Config file: $ENV_FILE"
  echo ""
  echo "To start: source $MEKONG_HOME/scripts/shell-init.sh"
  echo "First command: mekong $(echo 'okr,swot,board-report,annual,ipo-readiness-check' | cut -d',' -f1)"
...
main() {
  ...
  if [ "$mode" = "--dry-run" ]; then
    ...
    echo "  mekong $(echo 'okr,swot,board-report,annual,ipo-readiness-check' | cut -d',' -f1)"
  fi
}
