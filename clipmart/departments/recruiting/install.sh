#!/usr/bin/env bash
# Mekong CLI — Install dept-recruiting
set -euo pipefail

DEPT_ID="dept-recruiting"
DEPT_NAME="recruiting"
...
dry_run() {
  ...
  echo "Commands available: recruiter-screen,recruiter-source,hr-recruit"
  echo "Skills available: hr-agent"
  echo 'Pricing: $1.50 per candidate_screened (floor $49/mo)'
  echo "Config file: $ENV_FILE"
  echo ""
  echo "To start: source $MEKONG_HOME/scripts/shell-init.sh"
  echo "First command: mekong $(echo 'recruiter-screen,recruiter-source,hr-recruit' | cut -d',' -f1)"
...
main() {
  ...
  if [ "$mode" = "--dry-run" ]; then
    ...
    echo "  mekong $(echo 'recruiter-screen,recruiter-source,hr-recruit' | cut -d',' -f1)"
  fi
}
