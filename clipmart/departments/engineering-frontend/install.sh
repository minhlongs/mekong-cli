#!/usr/bin/env bash
# Mekong CLI — Install dept-engineering-frontend
set -euo pipefail

DEPT_ID="dept-engineering-frontend"
DEPT_NAME="engineering-frontend"
...
dry_run() {
  ...
  echo "Commands available: frontend-ui-build,frontend-responsive-fix,cook"
  echo "Skills available: frontend,frontend-development"
  echo 'Pricing: $5.00 per frontend_task (floor $49/mo)'
  echo "Config file: $ENV_FILE"
  echo ""
  echo "To start: source $MEKONG_HOME/scripts/shell-init.sh"
  echo "First command: mekong $(echo 'frontend-ui-build,frontend-responsive-fix,cook' | cut -d',' -f1)"
...
main() {
  ...
  if [ "$mode" = "--dry-run" ]; then
    ...
    echo "  mekong $(echo 'frontend-ui-build,frontend-responsive-fix,cook' | cut -d',' -f1)"
  fi
}
