#!/usr/bin/env bash
# Mekong CLI — Install dept-cx-support
set -euo pipefail

DEPT_ID="dept-cx-support"
DEPT_NAME="cx-support"
...
dry_run() {
  ...
  echo "Commands available: business-client-onboard,business-revenue-engine"
  echo "Skills available: customer-data-unification"
  echo 'Pricing: $0.75 per ticket_resolved (floor $49/mo)'
  echo "Config file: $ENV_FILE"
  echo ""
  echo "To start: source $MEKONG_HOME/scripts/shell-init.sh"
  echo "First command: mekong $(echo 'business-client-onboard,business-revenue-engine' | cut -d',' -f1)"
...
main() {
  ...
  if [ "$mode" = "--dry-run" ]; then
    ...
    echo "  mekong $(echo 'business-client-onboard,business-revenue-engine' | cut -d',' -f1)"
  fi
}
