...
dry_run() {
  ...
  echo "Commands available: legal-contract-review,legal-policy"
  echo "Skills available: legal-agent,legal-compliance"
  echo 'Pricing: $12.00 per legal_document (floor $49/mo)'
  echo "Config file: $ENV_FILE"
  echo ""
  echo "To start: source $MEKONG_HOME/scripts/shell-init.sh"
  echo "First command: mekong $(echo 'legal-contract-review,legal-policy' | cut -d',' -f1)"
...
main() {
  ...
  if [ "$mode" = "--dry-run" ]; then
    ...
    echo "  mekong $(echo 'legal-contract-review,legal-policy' | cut -d',' -f1)"
  fi
}
