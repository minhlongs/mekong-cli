#!/usr/bin/env bash
# Mekong CLI — Install dept-engineering-backend
set -euo pipefail

DEPT_ID="dept-engineering-backend"
DEPT_NAME="engineering-backend"
...
dry_run() {
  ...
  echo "Commands available: backend-api-build,backend-db-task,cook"
  echo "Skills available: backend,backend-development"
  echo 'Pricing: $5.00 per engineering_task (floor $49/mo)'
  echo "Config file: $ENV_FILE"
  echo ""
  echo "To start: source $MEKONG_HOME/scripts/shell-init.sh"
  echo "First command: mekong $(echo 'backend-api-build,backend-db-task,cook' | cut -d',' -f1)"
...
main() {
  ...
  if [ "$mode" = "--dry-run" ]; then
    ...
    echo "  mekong $(echo 'backend-api-build,backend-db-task,cook' | cut -d',' -f1)"
  fi
}
