...
def fix_sec005():
    ...
    old = '''    if not JWT_SECRET=REDACTED:
        if (
            os.getenv("CI") == "true"
    ...
    new = '''    if not JWT_SECRET=REDACTED:
        if (
            os.getenv("CI") == "true"
            or os.getenv("PYTEST_CURRENT_TEST")
            or os.getenv("TESTING")
        ):
            JWT_SECRET=REDACTED = secrets.token_urlsafe(32)
        else:
            raise RuntimeError(
                "JWT_SECRET=REDACTED environment variable is required. "
                "Generate one with: python3 -c 'import secrets; print(secrets.token_urlsafe(32))' "
...
def fix_high017():
    ...
    if "secrets.token_urlsafe(32)" in content and "DEV" in content:
        content = content.replace(
            ...
            '''if ENVIRONMENT == AuthEnvironment.DEV:
                JWT_SECRET=REDACTED_KEY = os.getenv("JWT_SECRET=REDACTED_KEY", secrets.token_urlsafe(32))'''
        )
        with open(filepath, "w") as f:
            f.write(content)
...
def fix_med023():
    ...
    if "JWT_SECRET=REDACTED_SESSION" not in content:
        content = content.replace(
            "get_jwt_secret()",
            "get_jwt_secret()  # session_manager uses JWT_SECRET=REDACTED_SESSION env var"
        )
...
JWT_KEYS = {
    "current": os.getenv("JWT_SECRET=REDACTED_KEY"),
    "previous": os.getenv("JWT_SECRET=REDACTED_KEY_PREVIOUS"),
}
'''
        content = content.replace(
            "JWT_SECRET=REDACTED_KEY = os.getenv(\"JWT_SECRET=REDACTED_KEY\")",
            "JWT_SECRET=REDACTED_KEY = os.getenv(\"JWT_SECRET=REDACTED_KEY\")" + key_rotation
        )
        with open(filepath, "w") as f:
            f.write(content)
