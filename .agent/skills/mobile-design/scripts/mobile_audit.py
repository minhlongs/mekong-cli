...
class MobileAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        if is_react_native:
            ...
            _ = bool(re.search(r'fontSize:\s*34|largeTitle|font-weight:\s*["\']?bold', content))
            _ = bool(re.search(r'fontSize:\s*28', content))
            _ = bool(re.search(r'fontSize:\s*17.*semibold|headline', content))
            _ = bool(re.search(r'fontSize:\s*17.*regular|body', content))
        ...
        if is_react_native:
            ...
            _ = bool(re.search(r'fontSize:\s*2[12][0-9].*medium|title', content))
            _ = bool(re.search(r'fontSize:\s*1[456].*regular|body', content))
        ...
        _ = re.findall(r'#[0-9A-Fa-f]{6}|rgba?\([^)]+\)', content)
        ...
        if is_react_native:
            ...
            _ = bool(re.search(r'systemBackground|\.systemBackground', content))
        ...
        _ = bool(re.search(r'debugger|__DEV__|React\.DevTools', content))
