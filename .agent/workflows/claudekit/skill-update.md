---
description: description: Update an existing agent skill [auto]
---

# Claudekit Command: /skill-update
...
## Your mission
Update an existing skill or its reference files in `$HOME/.claude/skills/${SKILL}` directory based on the user's prompt. 
...
### Scopes
- Project-scope: Current working project directory (e.g. `$HOME/.claude/`)
- User-scope: Home/user directory (e.g. `~/.claude/`)

## IMPORTANT NOTES:
- ALWAYS make changes to skills in the project-scope `$HOME/.claude/skills/` directory (UNLESS you're allowed to).
