...
**IMPORTANT**: Analyze the list of skills  at `.agencyos/skills/*` and intelligently activate the skills that are needed for the task during the process.
...
```bash
node .agencyos/scripts/set-active-plan.cjs plans/{your-folder-name}
```

Example:
```bash
node .agencyos/scripts/set-active-plan.cjs plans/251201-1430-GH88-add-authentication
```
