...
```bash
cd .agencyos/skills/mcp-management/scripts
npm install
```

### 2. Configure MCP Servers

Create `.agencyos/.mcp.json`:
...
See `.agencyos/.mcp.json.example` for more examples.
...
- Load config from `.agencyos/.mcp.json`
...
2. `.agencyos/skills/mcp-management/.env`
3. `.agencyos/skills/.env`
4. `.agencyos/.env`
...
The `mcp-manager` agent (`.agencyos/agents/mcp-manager.md`) uses this skill to:
...
Ensure `.agencyos/.mcp.json` exists and is valid JSON.
