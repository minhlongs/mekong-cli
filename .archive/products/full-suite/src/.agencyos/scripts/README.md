...
2. **PROJECT/.agencyos/skills/\<skill\>/.env** - Project skill-specific
3. **PROJECT/.agencyos/skills/.env** - Project shared across skills
4. **PROJECT/.agencyos/.env** - Project global defaults
5. **~/.agencyos/skills/\<skill\>/.env** - User skill-specific
6. **~/.agencyos/skills/.env** - User shared across skills
7. **~/.agencyos/.env** - User global defaults (LOWEST)

### CLI Usage

```bash
# Resolve a variable for a specific skill
python ~/.agencyos/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal

# With verbose output
python ~/.agencyos/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal --verbose

# Find all locations where variable is defined
python ~/.agencyos/scripts/resolve_env.py GEMINI_API_KEY --find-all

# Show hierarchy for a skill
python ~/.agencyos/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal

# Export format for shell sourcing
eval $(python ~/.agencyos/scripts/resolve_env.py GEMINI_API_KEY --export)
```
...
sys.path.insert(0, str(Path.home() / '.agencyos' / 'scripts'))
...
if not api_key:
    ...
    print("Run: python ~/.agencyos/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal")
    sys.exit(1)
...
python ~/.agencyos/scripts/resolve_env.py TEST_VAR --verbose
...
python ~/.agencyos/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal --find-all
```
...
python .agencyos/scripts/generate_catalogs.py --skills

# Generate commands catalog (outputs to stdout)
python .agencyos/scripts/generate_catalogs.py --commands

# Generate both catalogs (outputs to stdout)
python .agencyos/scripts/generate_catalogs.py

# Write to file instead of stdout
python .agencyos/scripts/generate_catalogs.py --skills --output guide/SKILLS.yaml

# View help
python .agencyos/scripts/generate_catalogs.py --help
```
